...
export default function EmployeeContractPage() {
  ...
  if (loading) {
    return <div className="flex justify-center py-24"><Loader2 className="w-6 h-6 text-[var(--brand)] animate-spin" /></div>;
  }

  if (!employee) {
    return <div className="text-center py-24 text-[var(--text-muted)] text-sm">Employé introuvable</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* Carte employé */}
      <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] shadow-sm p-6 flex flex-col sm:flex-row sm:items-center gap-5">
        <div className="w-14 h-14 rounded-2xl bg-[var(--brand-soft)] text-[var(--brand)] flex items-center justify-center font-black text-lg shrink-0">
          {employee.firstName[0]}{employee.lastName[0]}
        </div>
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-black text-[var(--text)]">{employee.firstName} {employee.lastName}</h1>
        ...
        <button onClick={() => router.push(bp(`/contrats/generer?employeeId=${employee.id}`))}
          className="flex items-center gap-2 px-4 py-2.5 bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white text-sm font-bold rounded-xl shadow-md transition-all shrink-0">
      ...
      <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] shadow-sm p-6">
        <p className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4 flex items-center gap-1.5">
      ...
      {latest && (
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-2">Dernier document généré</p>
      ...
      <div>
        ...
        {contracts.length === 0 ? (
          <div className="bg-[var(--surface)] rounded-2xl border border-dashed border-[var(--border)] p-10 text-center">
            <FileX2 className="w-8 h-8 text-[var(--text-muted)] opacity-50 mx-auto mb-3" />
            <p className="text-sm font-semibold text-[var(--text)]">Aucun contrat généré pour cet employé</p>
            <p className="text-xs text-[var(--text-muted)] mt-1 mb-4">Générez son premier contrat en quelques secondes.</p>
            <button onClick={() => router.push(bp(`/contrats/generer?employeeId=${employee.id}`))}
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white text-sm font-bold rounded-xl shadow-md transition-all">
...
function InfoTile({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div>
      <p className="flex items-center gap-1 text-[11px] text-[var(--text-muted)] mb-1"><Icon className="w-3 h-3" /> {label}</p>
...
function ContractCard({ contract, featured }: { contract: GeneratedContract; featured?: boolean }) {
  ...
  const downloadDocx = async () => {
    setDownloading(true);
    try {
      const blob = await api.getBlob(`/contracts/generation/${contract.id}/download`);
  ...
  const openPdf = async () => {
    setPreviewing(true);
    try {
      const blob = await api.getBlob(`/contracts/generation/${contract.id}/preview`);
      const url = URL.createObjectURL(blob);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      className={`rounded-2xl border p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 shadow-sm ${
        featured
          ? 'bg-[var(--brand)] border-transparent text-white'
          : 'bg-[var(--surface)] border-[var(--border)]'
      }`}>
      <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${featured ? 'bg-white/15' : 'bg-[var(--brand-soft)]'}`}>
        <FileSignature className={`w-5 h-5 ${featured ? 'text-white' : 'text-[var(--brand)]'}`} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className={`text-sm font-bold ${featured ? 'text-white' : 'text-[var(--text)]'}`}>
            {KIND_LABEL[contract.kind] || contract.kind}
          </p>
          <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${featured ? 'bg-white/15 text-white' : 'bg-[var(--surface-2)] text-[var(--text-muted)]'}`}>
            {contract.contractDuration === 'DETERMINEE' ? 'Déterminée' : 'Indéterminée'}
          </span>
        </div>
        <div className={`flex items-center gap-3 mt-1 text-xs flex-wrap ${featured ? 'text-white/70' : 'text-[var(--text-muted)]'}`}>
      ...
      <div className="flex gap-2 shrink-0">
        <button onClick={openPdf} disabled={previewing}
          className={`flex items-center gap-1.5 px-3 py-2 text-xs font-bold rounded-lg transition-colors disabled:opacity-60 ${featured ? 'bg-white/15 hover:bg-white/25 text-white' : 'bg-[var(--surface-2)] hover:bg-[var(--border)] text-[var(--text)]'}`}>
        ...
        <button onClick={downloadDocx} disabled={downloading}
          className={`flex items-center gap-1.5 px-3 py-2 text-xs font-bold rounded-lg transition-colors disabled:opacity-60 ${featured ? 'bg-white text-[var(--brand)] hover:bg-white/90' : 'bg-[var(--brand)] hover:bg-[var(--brand-strong)] text-white'}`}>
