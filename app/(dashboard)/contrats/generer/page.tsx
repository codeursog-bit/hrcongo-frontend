...
function GenerateContractInner() {
  ...
  const downloadDocx = async (id: string) => {
    ...
    try {
      const blob = await api.getBlob(`/contracts/generation/${id}/download`);
  ...
  const openPdf = async (id: string) => {
    setOpeningPdf(true);
    try {
      const blob = await api.getBlob(`/contracts/generation/${id}/preview`);
      const url = URL.createObjectURL(blob);
                ...
                {form.employeeId && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4 overflow-hidden">
                    <div className="grid sm:grid-cols-3 gap-3">
                      <Field label={isTravail ? 'Type de contrat' : isStage ? 'Durée du stage' : 'Durée de la mission'}>
                        <div className="relative">
