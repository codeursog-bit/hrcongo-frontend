...
const ST={OUVERT:{l:'Ouvert',c:'text-amber-400',bg:'bg-amber-500/10',b:'border-amber-500/20'},EN_COURS:{l:'En cours',c:'text-amber-400',bg:'bg-amber-500/10',b:'border-amber-500/20'},CLOTURE:{l:'Clôturé',c:'text-emerald-400',bg:'bg-emerald-500/10',b:'border-emerald-500/20'},ANNULE:{l:'Annulé',c:'text-red-400',bg:'bg-red-500/10',b:'border-red-500/20'}};
const fmtD=(d?:string)=>d?new Date(d).toLocaleDateString('fr-FR',{day:'2-digit',month:'short',year:'numeric'}):'—';
function Sp(){return<div className="w-4 h-4 rounded-full border-2 border-emerald-500/30 border-t-emerald-500 animate-spin"/>}
function Bdg({c,v='d'}:{c:React.ReactNode;v?:'d'|'s'|'w'|'r'|'i'}){const m={d:'bg-slate-800 text-slate-300 border-slate-700',s:'bg-emerald-500/10 text-emerald-400 border-emerald-500/20',w:'bg-amber-500/10 text-amber-400 border-amber-500/20',r:'bg-red-500/10 text-red-400 border-red-500/20',i:'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}[v];return<span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${m}`}>{c}</span>}
...
function NouvellePSE({onClose,onCreated}:{onClose:()=>void;onCreated:()=>void}){
  ...
  return<div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4"><div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-xl max-h-[85vh] overflow-y-auto shadow-2xl">
    ...
    <div className="mb-5 p-3 bg-[var(--accent-2-soft)] border border-amber-500/20 rounded-xl text-xs text-amber-300 leading-relaxed"><strong>Obligations légales :</strong> Notification à l'Inspection du Travail obligatoire avant toute notification aux salariés. Délai minimum : 15 jours ouvrables.</div>
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="space-y-1.5 max-h-48 overflow-y-auto no-scrollbar">{filtered.map(e=>{const s=form.salariesIds.includes(e.id);return<button key={e.id} onClick={()=>toggle(e.id)} className={`w-full flex items-center gap-3 p-2.5 rounded-xl border text-left transition-all ${s?'bg-emerald-500/10 border-emerald-500/30':'bg-slate-800/40 border-white/5 hover:border-white/15'}`}><div className={`w-4 h-4 rounded flex items-center justify-center border shrink-0 transition-all ${s?'bg-emerald-500 border-emerald-500':'bg-slate-800 border-slate-600'}`}>{s&&<svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>}</div><div className="min-w-0"><p className="text-xs font-semibold text-slate-200 truncate">{e.firstName} {e.lastName}</p><p className="text-xs text-slate-500">{e.employeeNumber} · {e.position}</p></div></button>})}</div>
      </div>
    ...
    <button onClick={submit} disabled={loading||!form.motif.trim()} className={`mt-4 w-full py-3 rounded-xl font-bold text-sm transition-all ${form.motif.trim()&&!loading?'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20':'bg-slate-800 text-slate-600 cursor-not-allowed'}`}>{loading?'Ouverture...':'Ouvrir la procédure PSE'}</button>
  </div></div>;
}
...
function PSECard({pse,onUpdate}:{pse:PSE;onUpdate:()=>void}){
  ...
  const barC=pse.status==='CLOTURE'?'bg-emerald-500':'bg-amber-500';
  ...
  return<div className={`bg-slate-900/60 border rounded-2xl overflow-hidden transition-all ${st.b}`}>
    ...
    {open&&<div className="border-t border-white/10 p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
      ...
      <div className="space-y-4">
        ...
        {pse.motif&&<div className="p-3 bg-[var(--accent-2-soft)] border border-amber-500/20 rounded-xl text-xs text-amber-300 leading-relaxed"><strong>Motif :</strong> {pse.motif}</div>}
      </div>
    </div>}
  </div>;
}
  ...
  return<div className="min-h-screen bg-[#020617] text-slate-100">
    ...
    <div className="sticky top-0 z-30 bg-[#020617]/90 backdrop-blur-xl border-b border-white/[0.06]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-3">
        <div className="w-7 h-7 rounded-lg bg-emerald-600 flex items-center justify-center shadow-lg shadow-emerald-500/30 shrink-0"><svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M3 6h18M3 12h18M3 18h18"/></svg></div>
        ...
        <button onClick={()=>setShowModal(true)} className="bg-emerald-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg shadow-emerald-500/20 shrink-0">+ Ouvrir PSE</button>
      </div>
    </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5">
      ...
      {/* Alerte */}
      {stats?.pseRequired&&<div className="p-4 sm:p-5 bg-red-950/60 border border-red-500/30 rounded-2xl flex gap-4 items-start">
        <div className="w-8 h-8 rounded-xl bg-red-500/20 border border-red-500/30 flex items-center justify-center shrink-0"><svg className="w-4 h-4 text-red-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg></div>
        <div><p className="text-sm font-bold text-red-300 mb-1">Procédure PSE obligatoire — Art. 39 CT Congo</p><p className="text-xs text-slate-400 leading-relaxed"><strong className="text-slate-200">{stats.total30j} licenciements économiques</strong> sur 30 jours — seuil légal de <strong className="text-slate-200">5</strong> dépassé. Toute rupture supplémentaire requiert l'autorisation préalable de la Commission des Litiges du Travail.</p><button onClick={()=>setShowModal(true)} className="mt-2 text-xs font-bold text-red-400 hover:text-red-300 underline">Ouvrir une procédure PSE</button></div>
      </div>}

      {/* Tabs */}
      <div className="flex bg-slate-800/60 rounded-xl p-0.5 gap-0.5 w-fit">
        {([{k:'actifs'as const,l:`Actives (${actifs.length})`},{k:'historique'as const,l:`Historique (${historique.length})`}]).map(t=><button key={t.k} onClick={()=>setTab(t.k)} className={`px-3 py-1.5 rounded-[10px] text-xs font-bold transition-all ${tab===t.k?'bg-emerald-600 text-white':'text-slate-500 hover:text-slate-200'}`}>{t.l}</button>)}
      </div>
      ...
      {loading?<div className="flex justify-center py-12"><Sp/></div>:displayed.length===0?<div className="text-center py-16 bg-slate-900/40 border border-white/10 border-dashed rounded-2xl">
        ...
        {tab==='actifs'&&<button onClick={()=>setShowModal(true)} className="text-xs font-bold text-emerald-400 hover:text-emerald-300 border border-emerald-500/20 px-3 py-1.5 rounded-lg transition-all">Ouvrir manuellement</button>}
