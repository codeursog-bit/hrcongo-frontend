...
//
// 🎨 Refonte design : palette émeraude/ambre + tokens CSS.
//    Les classes de teinte étaient construites dynamiquement (`bg-${color}-100`),
//    ce que Tailwind ne peut pas générer au build → remplacées par des classes
//    statiques via TINT.
// ============================================================================
...
// 2 teintes seulement : ambre = interruption non planifiée, émeraude = demande planifiée / financière
const TYPE_META: Record<'absence' | 'permission' | 'leave' | 'loan', { label: string; icon: any; tint: 'amber' | 'emerald'; href: string }> = {
  absence:    { label: 'Absence',    icon: FileText,  tint: 'amber',   href: '/presences/absences' },
  permission: { label: 'Permission', icon: Ticket,    tint: 'amber',   href: '/presences/permissions' },
  leave:      { label: 'Congé',      icon: Umbrella,  tint: 'emerald', href: '/conges' },
  loan:       { label: 'Prêt/Avance', icon: HandCoins, tint: 'emerald', href: '/loans' },
};

// Classes statiques — indispensable pour que Tailwind les génère
const TINT: Record<'amber' | 'emerald', string> = {
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-24 space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-[var(--text)]">Demandes</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {([
          ...
          return (
            <Link key={kind} href={bp(meta.href)} className="group bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-5 hover:border-emerald-500/40 transition-colors">
              <div className="flex items-center justify-between mb-4">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${TINT[meta.tint]}`}>
                  <Icon size={20} />
                </div>
                {count > 0 && (
              ...
              <p className="text-sm text-[var(--text-muted)] mb-3">{meta.label}{count > 1 ? 's' : ''} à traiter · {total} au total</p>
              <span className="text-xs font-bold text-emerald-500 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] overflow-hidden">
        <div className="p-5 border-b border-[var(--border)]">
          <h2 className="font-bold text-[var(--text)] flex items-center gap-2">
            <Clock size={16} className="text-[var(--text-muted)]" /> Toutes les demandes en attente
          </h2>
        </div>
        {combinedPending.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-sm text-[var(--text-muted)]">Aucune demande en attente — tout est traité 🎉</p>
          </div>
        ) : (
          <div className="divide-y divide-[var(--border)]">
            {combinedPending.map(item => {
              ...
              return (
                <Link key={`${item.kind}-${item.id}`} href={bp(meta.href)} className="flex items-center gap-3 p-4 hover:bg-[var(--surface-2)] transition-colors">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${TINT[meta.tint]}`}>
                    <SubIcon size={15} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[var(--text)] truncate">{item.employee?.firstName} {item.employee?.lastName}</p>
                    <p className="text-xs text-[var(--text-muted)]">{meta.label} · {new Date(item.date).toLocaleDateString('fr-FR')}</p>
                  </div>
