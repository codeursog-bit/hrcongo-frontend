...
export default function EmployeeDebtDetailPage() {
  ...
  const trendData = useMemo(() => {
    ...
    return months.map(m => {
      // ✅ Un prêt se classe sur sa date de DÉPART voulue (startDate), pas
      // createdAt (date de création de l'enregistrement, souvent "aujourd'hui"
      // même si le prêt a été programmé pour un mois différent) — c'est ce
      // qui faisait paraître la courbe "figée" : tout nouveau prêt atterrissait
      // toujours dans le mois réel du jour, jamais dans le mois choisi. Une
      // avance n'a pas de date de départ distincte : createdAt reste la
      // meilleure approximation de son décaissement réel.
      const emprunte = history.filter(r => {
        const d = new Date(r.kind === 'loan' ? (r.startDate ?? r.createdAt) : r.createdAt);
