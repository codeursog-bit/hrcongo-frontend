    ...
    <div className="max-w-[1500px] mx-auto pb-24 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Le solde restant est toujours à jour ; "accordé" et "remboursé" reflètent l'activité de la période affichée.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-900">
              <tr>{['Employé', 'Département', 'Type(s)', `Accordé (${month !== '' ? MONTHS_FR[month - 1] : year})`, `Remboursé (${month !== '' ? MONTHS_FR[month - 1] : year})`, 'Solde restant (à ce jour)', ''].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-bold text-gray-400 uppercase whitespace-nowrap">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {byEmployee.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-14 text-gray-400">Aucun employé avec un prêt ou une avance.</td></tr>
              ) : byEmployee.map(e => (
                <tr key={e.employeeId} className="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                  ...
                  <td className="px-4 py-3 text-gray-500">{Array.from(e.types as Set<string>).map((t: string) => TYPE_LABEL[t] ?? t).join(', ') || '—'}</td>
                  <td className="px-4 py-3 font-semibold text-gray-800 dark:text-gray-100 whitespace-nowrap">{e.givenInPeriod > 0 ? fmt(e.givenInPeriod) : '—'}</td>
