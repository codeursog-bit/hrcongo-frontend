...
// // 📁 src/app/(dashboard)/paie/[id]/components/PayslipBreakdown.tsx
// //
// // ✅ PRINCIPE : ZÉRO calcul côté front
// //    Toutes les données viennent du backend (payroll-calculator.service.ts)
// //    Ce composant AFFICHE uniquement.
// //
// // ✅ NOUVEAU : Section "Part Patronale" avec les 3 branches CNSS + TUS
// //    Données reçues via props depuis la BDD ou via irppDetails (fallback)
// // ============================================================================
...
//   absenceDays?: number;
//   workDays?: number;
//   // ✅ Données patronales — calculées par le backend
//   cnssEmployer?: number;
//   cnssEmployerPension?: number;
//   cnssEmployerFamily?: number;
//   cnssEmployerAccident?: number;
...
//   bonuses = [], absenceDeduction = 0, absenceDays = 0, workDays = 26,
//   cnssEmployer = 0,
//   cnssEmployerPension: cnssEmployerPensionProp,
//   cnssEmployerFamily: cnssEmployerFamilyProp,
//   cnssEmployerAccident: cnssEmployerAccidentProp,
//   tusAmount: tusAmountProp,
//   totalEmployerCost = 0,
//   irppDetails,
// }: Props) {
//   const fmt = (v: number) => Math.round(v ?? 0).toLocaleString('fr-FR');

//   // ── Détail patronal — backend en priorité, irppDetails en fallback ─────────
//   // Anciens bulletins sans les nouveaux champs : on lit irppDetails
//   const cnssEmployerPension  = cnssEmployerPensionProp  ?? irppDetails?.cnssEmployerDetail?.pension  ?? 0;
//   const cnssEmployerFamily   = cnssEmployerFamilyProp   ?? irppDetails?.cnssEmployerDetail?.famille  ?? 0;
//   const cnssEmployerAccident = cnssEmployerAccidentProp ?? irppDetails?.cnssEmployerDetail?.accident ?? 0;
//   const tusAmount            = tusAmountProp            ?? irppDetails?.tus                          ?? 0;
//   const cnssPatTotal         = cnssEmployerPension + cnssEmployerFamily + cnssEmployerAccident;
...
//             {/* ✅ PART PATRONALE — données 100% backend */}
//             <Head label="Part Patronale — Charges Sociales Employeur" bg={C.orange} />
//             {(() => { rowIdx = 0; return null; })()}
//             <Row label="CNSS Patronale — Pensions (Vieillesse / Invalidité)"
//               sub="Plafond mensuel : 1 200 000 FCFA"
...
//             <Row label="CNSS Patronale — Prestations Familiales"
//               sub="Plafond mensuel : 600 000 FCFA"
//               rate="10 %" amount={`+${fmt(cnssEmployerFamily)}`} amtColor={C.orange} />
//             <Row label="CNSS Patronale — Accidents du Travail"
//               sub="Plafond mensuel : 600 000 FCFA"
//               rate="2,25 %" amount={`+${fmt(cnssEmployerAccident)}`} amtColor={C.orange} />
//             <Row label="TUS — Taxe Unique sur les Salaires"
//               sub="Patronale · calculée sur brut total · déplafonné"
//               rate="2 %" amount={`+${fmt(tusAmount)}`} amtColor={C.orange} />

//             {/* TOTAL PATRONAL */}
//             <tr>
//               <td colSpan={3} style={{ background: C.orangeLt, padding: '9px 14px', borderTop: '2px solid #fed7aa', fontSize: '10.5px', fontWeight: 700, textTransform: 'uppercase', color: C.orange }}>Total Charges Patronales</td>
//               <td style={{ background: C.orangeLt, padding: '9px 14px', borderTop: '2px solid #fed7aa', textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, fontSize: '12px', color: C.orange }}>+{fmt(cnssPatTotal + tusAmount)}</td>
//             </tr>
...
//           <div style={{ background: C.white, padding: '10px 14px' }}>
//             {[{ label: 'CNSS salariale (4 %)', val: `−${fmt(cnssAmt)} F`, color: C.red },
//               { label: 'ITS — barème 2026',    val: `−${fmt(irppAmt)} F`, color: C.red }].map(r => (
//               <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
//                 <span style={{ fontSize: '10px', color: '#6b7280' }}>{r.label}</span>
//                 <span style={{ fontSize: '10px', fontFamily: 'monospace', fontWeight: 700, color: r.color }}>{r.val}</span>
//               </div>
...
//           </div>
//           <div style={{ background: C.white, padding: '10px 14px' }}>
//             {[{ label: 'Pensions 8% / plaf. 1 200k',   val: `+${fmt(cnssEmployerPension)} F`,   color: C.orange },
//               { label: 'Famille 10% / plaf. 600k',      val: `+${fmt(cnssEmployerFamily)} F`,    color: C.orange },
//               { label: 'Acc. travail 2,25% / 600k',     val: `+${fmt(cnssEmployerAccident)} F`,  color: C.orange },
//               { label: 'TUS 2% (déplafonné)',            val: `+${fmt(tusAmount)} F`,             color: C.orange }].map(r => (
//               <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
//                 <span style={{ fontSize: '9.5px', color: '#6b7280' }}>{r.label}</span>
//                 <span style={{ fontSize: '10px', fontFamily: 'monospace', fontWeight: 700, color: r.color }}>{r.val}</span>
//               </div>
...
//             <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 7, borderTop: `1px solid ${C.border}` }}>
//               <span style={{ fontSize: '10.5px', fontWeight: 700, color: C.text }}>Total patronal</span>
//               <span style={{ fontSize: '11px', fontFamily: 'monospace', fontWeight: 700, color: C.orange }}>{fmt(cnssPatTotal + tusAmount)} F</span>
//             </div>
//           </div>
//         </div>
...
//             {[{ label: 'Salaire brut',       val: `+${fmt(grossSalary)} F`,             color: C.green  },
//               { label: 'Total retenues',     val: `−${fmt(totalRet)} F`,                color: C.red    },
//               { label: 'Charges patronales', val: `+${fmt(cnssPatTotal + tusAmount)} F`, color: C.orange }].map(r => (
//               <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
//                 <span style={{ fontSize: '10px', color: '#6b7280' }}>{r.label}</span>
//                 <span style={{ fontSize: '10px', fontFamily: 'monospace', fontWeight: 700, color: r.color }}>{r.val}</span>
//               </div>
