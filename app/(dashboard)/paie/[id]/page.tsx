...
//   };
//   company?: { legalName?: string; logo?: string; address?: string; rccmNumber?: string; nif?: string; phone?: string; collectiveAgreement?: string; [key: string]: any; };
//   items?: Array<{
...
//   // ✅ TUS — 3 champs séparés
//   tusDgiAmount?: number;   // 4,13% → DGI
//   tusCnssAmount?: number;  // 3,38% → CNSS
...
//   // TUS total pour la sidebar
//   const tusTotal      = data.tusTotal      ?? 0;
//   const tusDgiAmount  = data.tusDgiAmount  ?? 0;
//   const tusCnssAmount = data.tusCnssAmount ?? 0;
//   const cnssPatTotal  = (data.cnssEmployerPension ?? 0) + (data.cnssEmployerFamily ?? 0) + (data.cnssEmployerAccident ?? 0);
...
//             <PayslipHeader company={data.company ?? {}} month={data.month} year={data.year} status={data.status} />
//             <PayslipEmployeeInfo
...
//                 // ✅ TUS — 3 champs
//                 tusDgiAmount={tusDgiAmount}
//                 tusCnssAmount={tusCnssAmount}
//                 tusTotal={tusTotal}
...
//                         </div>
//                         <div className="flex justify-between text-gray-500">
//                           <span>TUS DGI (4,13%)</span>
//                           <span className="font-mono font-bold text-orange-500">+{fmt(tusDgiAmount)} F</span>
//                         </div>
//                         <div className="flex justify-between text-gray-500">
//                           <span>TUS CNSS (3,38%)</span>
//                           <span className="font-mono font-bold text-orange-500">+{fmt(tusCnssAmount)} F</span>
//                         </div>
...
//             {/* Réglementation Congo 2026 — corrigée */}
...
//                 <p>• ITS 2026 — barème progressif 1%/10%/25%/40%</p>
...
//                 <p className="font-semibold text-amber-600 dark:text-amber-400">• TUS DGI : 4,13% sur brut (déplafonné)</p>
//                 <p className="font-semibold text-amber-600 dark:text-amber-400">• TUS CNSS : 3,38% sur brut (déplafonné)</p>
...
interface PayrollData {
  ...
  cnssEmployerAccident?: number;
  // ✅ TUS — 3 champs séparés
  tusDgiAmount?: number;   // 2,025% → DGI
  tusCnssAmount?: number;  // 5,475% → CNSS
  tusTotal?: number;       // 7,51% total
  irppDetails?: any;
}
...
export default function PayslipPage({ params }: { params: { id: string } }) {
  ...
  const fmt = (v: number | string | any) => (Number(v) || 0).toLocaleString('fr-FR');
  const isAdmin = ['SUPER_ADMIN', 'ADMIN', 'HR_MANAGER'].includes(userRole || '');
  const isDraft = data?.status === 'DRAFT';
  ...
  const cnssPatTotal  = Number(data.cnssEmployerPension ?? 0) + Number(data.cnssEmployerFamily ?? 0) + Number(data.cnssEmployerAccident ?? 0);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_290px] gap-6 items-start print:block">
          ...
          <div className="no-print space-y-4">
                  ...
                  {showEmployer && (
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }}
                      ...
                      <div className="p-4 space-y-2 text-sm bg-gray-50 dark:bg-gray-900/40">
                        ...
                        <div className="flex justify-between text-gray-500">
                          <span>TUS DGI (2,025%)</span>
                          <span className="font-mono font-bold text-orange-500">+{fmt(tusDgiAmount)} F</span>
                        </div>
                        <div className="flex justify-between text-gray-500">
                          <span>TUS CNSS (5,475%)</span>
                          <span className="font-mono font-bold text-orange-500">+{fmt(tusCnssAmount)} F</span>
                        </div>
                        <div className="flex justify-between font-bold pt-2 border-t border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white">
                          <span>Coût Total</span>
            ...
            <div className="bg-sky-50 dark:bg-sky-900/20 rounded-2xl p-4 border border-sky-200 dark:border-sky-800">
              ...
              <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
                <p>• ITS 2026 — barème progressif 1 200F/10%/15%/20%/30%</p>
                <p>• Abattement 20% avant calcul ITS</p>
                <p>• CNSS salarié : 4% (plafond 1 200 000 F)</p>
                <p>• CNSS pat. Pensions : 8% / 1 200 000 F</p>
                <p>• CNSS pat. Famille : 10,03% / 600 000 F</p>
                <p>• CNSS pat. Accident : 2,25% / 600 000 F</p>
                <p className="font-semibold text-amber-600 dark:text-amber-400">• TUS DGI : 2,025% sur brut (déplafonné)</p>
                <p className="font-semibold text-amber-600 dark:text-amber-400">• TUS CNSS : 5,475% sur brut (déplafonné)</p>
                <p>• SMIG : 70 400 FCFA/mois</p>
              </div>
            </div>
          </div>
