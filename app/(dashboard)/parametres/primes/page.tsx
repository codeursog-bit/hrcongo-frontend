...
import {
  ...
  TrendingUp, Award, Shield, DollarSign, Calendar,
  Zap, Hash, ToggleLeft
} from 'lucide-react';
...
export interface BonusTemplate {
  id:                 string;
  name:               string;
  bonusCategory?:     'FRAIS' | 'POSTE' | 'PERFORMANCE' | 'EXCEPTIONNELLE';
...
const CONVENTIONAL_PRESETS: Omit<BonusTemplate, 'id' | 'createdAt' | 'isActive'>[] = [
  {
    ...
    // ✅ Montant plein mois + prorata temporis (jours travaillés / jours
    // ouvrables de l'entreprise) — remplace l'ancien mode "AUTO_DAYS" cassé
    // et redondant : 26 000 FCFA le mois complet, 20 000 si 20/26 jours.
    defaultAmount: 26000, defaultPercentage: null, baseCalculation: null,
    ...
    description: 'Remboursement frais de transport — 26 000 FCFA le mois complet, réduit au prorata des jours travaillés',
  },
  {
    ...
    description: 'Indemnité repas — 2 000 FCFA × nombre de repas pris (ajustable chaque mois au moment de la paie)',
  },
  {
    name: 'Prime d\'assiduité',
    // ✅ Montant plein mois + prorata temporis (remplace l'ancien mode
    // "AUTO_WEEKS" : une absence complète d'une semaine entière est rare,
    // le prorata jour par jour couvre déjà largement ce cas)
    defaultAmount: 8000, defaultPercentage: null, baseCalculation: null,
    isRecurring: true, isTaxable: true, isCnss: false,
    fiscalType: 'TAXABLE_NO_CNSS',
    unitAmount: null, quantityMode: null, defaultQuantity: null, isProratized: true,
    description: 'Prime d\'assiduité — 8 000 FCFA le mois complet, réduite au prorata des jours travaillés',
  },
  {
  ...
  // 🆕 13e mois / prime de fin d'année — 3 modèles au choix (cf. discussion
  // sur le calcul : montant plein mois ou demi-mois du salaire de base,
  // versé en décembre, proratisé si embauche en cours d'année ; ou montant
  // fixe ressaisi chaque année si l'entreprise ne suit pas une formule)
  {
    name: "Prime de fin d'année (13e mois - mois complet)",
    bonusCategory: 'EXCEPTIONNELLE',
    defaultAmount: null, defaultPercentage: 100, baseCalculation: 'BASE_SALARY',
    isRecurring: true, isTaxable: true, isCnss: true,
    fiscalType: 'TAXABLE_CNSS',
    unitAmount: null, quantityMode: null, defaultQuantity: null, isProratized: true,
    description: "13e mois — 1 mois de salaire de base, versé en décembre. Réduit au prorata des mois travaillés si l'employé n'a pas fait l'année complète.",
  },
  {
    name: "Prime de fin d'année (13e mois - demi-mois)",
    bonusCategory: 'EXCEPTIONNELLE',
    defaultAmount: null, defaultPercentage: 50, baseCalculation: 'BASE_SALARY',
    isRecurring: true, isTaxable: true, isCnss: true,
    fiscalType: 'TAXABLE_CNSS',
    unitAmount: null, quantityMode: null, defaultQuantity: null, isProratized: true,
    description: "Demi-13e mois — 50% du salaire de base, versé en décembre. Réduit au prorata des mois travaillés si l'employé n'a pas fait l'année complète.",
  },
  {
    name: "Prime de fin d'année (montant fixe)",
    bonusCategory: 'EXCEPTIONNELLE',
    defaultAmount: null, defaultPercentage: null, baseCalculation: null,
    isRecurring: false, isTaxable: true, isCnss: true,
    fiscalType: 'TAXABLE_CNSS',
    unitAmount: null, quantityMode: null, defaultQuantity: null, isProratized: false,
    description: 'Montant que vous saisissez vous-même chaque année (ex: 100 000 FCFA), versé en décembre — sans formule automatique.',
  },
];

// 🆕 Labels et configs des modes de quantité
// 🆕 Deux modes seulement :
//  - null (montant fixe/%) : combiné à "isProratized" pour le prorata
//    temporis (montant plein mois ÷ jours ouvrables × jours travaillés) —
//    c'est le cas normal (transport, panier, ancienneté...)
//  - FREE (quantité libre) : le chef saisit/ajuste le montant du mois au
//    moment de la paie, sans affecter les autres mois (garde, prime
//    variable...). Les anciens modes AUTO_DAYS/AUTO_WEEKS/AUTO_HOURS ont été
//    retirés : redondants avec le prorata temporis et jamais fiabilisés.
const QUANTITY_MODES = [
  {
    value: null,
    label: 'Montant fixe / %',
    icon: DollarSign,
    desc: 'Montant plein mois — cochez "au prorata" pour le réduire selon les jours travaillés',
    color: 'slate',
  },
  {
    value: 'FREE',
    ...
    icon: Hash,
    desc: 'Le chef saisit/ajuste le montant du mois au moment de la paie (garde, prime variable...)',
    color: 'amber',
  },
] as const;
...
const emptyForm = () => ({
  name:              '',
  // 🆕 Catégorie — n'était jamais envoyée au backend (retombait toujours sur
  // 'PERFORMANCE' par défaut côté serveur), ce qui cassait la détection
  // "13e mois annuel" côté fiche employé. 'EXCEPTIONNELLE' pour le 13e
  // mois / fin d'année, sinon 'PERFORMANCE' par défaut comme avant.
  bonusCategory:     'PERFORMANCE' as 'FRAIS' | 'POSTE' | 'PERFORMANCE' | 'EXCEPTIONNELLE',
  ...
  // 🆕 Quantité
  quantityMode:      null as 'FREE' | null,
...
export default function CataloguePrimesPage() {
  ...
  const openEdit = (t: BonusTemplate) => {
    ...
    setForm({
      name:              t.name,
      bonusCategory:     t.bonusCategory ?? 'PERFORMANCE',
  ...
  const applyPreset = (p: Omit<BonusTemplate, 'id' | 'createdAt' | 'isActive'>) => {
    ...
    setForm({
      name:              p.name,
      bonusCategory:     p.bonusCategory ?? 'PERFORMANCE',
      fiscalType:        ft as any,
  ...
  const handleSave = async () => {
    ...
    const payload: any = {
      name:            form.name.trim(),
      bonusCategory:   form.bonusCategory,
