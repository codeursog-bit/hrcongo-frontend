...
// app/(dashboard)/portefeuille/conges/page.tsx

import React, { useEffect, useState } from 'react';
import {
  Calendar, Search, Loader2, AlertCircle, CheckCircle2, XCircle,
  Plus, Trash2, X, Ban,
} from 'lucide-react';
...
const LEAVE_TYPES = [
  ...
  { value: 'ANNUAL_ANTICIPATED', label: 'Congé annuel anticipé' },
];
...
function CreateModal({ companies, onClose, onCreated }: {
  ...
  const handleSubmit = async () => {
    if (!employee || !startDate || !endDate) { setError('Employé et dates sont obligatoires'); return; }
    setSaving(true);
    setError('');
    try {
      await api.post('/portfolio/leaves', { employeeId: employee.id, type, startDate, endDate, reason: reason || undefined });
      onCreated();
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg rounded-2xl overflow-hidden z-10" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
          <h2 className="text-sm font-bold" style={{ color: 'var(--text)' }}>Nouvelle demande de congé</h2>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          <Field label="Type de congé">
            <select value={type} onChange={e => setType(e.target.value)} className={inputCls} style={{ color: 'var(--text)' }}>
...
function DecisionModal({ item, approve, onClose, onDone }: {
  item: LeaveItem; approve: boolean; onClose: () => void; onDone: () => void;
}) {
  const [reason, setReason] = useState('');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      await api.patch(`/portfolio/leaves/${item.id}/status`, {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl p-6 z-10" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <p className="text-sm font-bold mb-1" style={{ color: 'var(--text)' }}>{approve ? 'Approuver' : 'Refuser'} ce congé ?</p>
...
export default function PortfolioLeavesPage() {
  ...
  const [decisionItem, setDecisionItem] = useState<{ item: LeaveItem; approve: boolean } | null>(null);
  ...
  const load = () => {
    ...
    api.get<LeaveItem[]>(`/portfolio/leaves?${params.toString()}`)
  ...
  const runAction = async (item: LeaveItem, action: 'cancel' | 'delete') => {
    setBusyId(item.id);
    setError('');
    try {
      if (action === 'delete') await api.delete(`/portfolio/leaves/${item.id}`);
      else await api.patch(`/portfolio/leaves/${item.id}/cancel`, {});
  ...
  return (
    <div className="space-y-6 min-h-screen pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: 'var(--text)' }}>Congés</h1>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center rounded-2xl" style={{ border: '2px dashed var(--border)' }}>
          <Calendar size={24} style={{ color: 'var(--text-muted)' }} className="mb-3" />
