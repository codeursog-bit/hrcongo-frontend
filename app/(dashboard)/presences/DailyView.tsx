...
export default function DailyView({ 
  ...
  const fetchDailyData = async () => {
    setIsLoading(true);
    try {
      // 🔒 Le backend filtre déjà par département si MANAGER — pas besoin de passer un filtre ici
      const res: any = await api.get(`/attendance?month=${selectedDate.getMonth() + 1}&year=${selectedDate.getFullYear()}`);
      setData(res);
    } catch (e) {
      console.error(e);
  ...
  const getDailyAttendances = () => {
    ...
    // On itère directement sur data.employees sans re-filtrer par userDepartment.
    data.employees.forEach((emp: any, empIndex: number) => {
      const empDayStatuses = data.dayStatuses[empIndex] || [];
      const dayStatus = empDayStatuses.find((ds: any) => ds.date === dateStr);
  ...
  const getFilteredAttendances = () => {
    ...
    // 🔒 MANAGER : aucun filtre front nécessaire — le backend a déjà restreint les données.
    // On applique uniquement les filtres UI (recherche, statut, département si ADMIN/HR).
    ...
    // Filtre département UI — disponible uniquement pour ADMIN/HR (non affiché pour MANAGER)
  ...
  // Départements disponibles — basés sur ce que le backend a renvoyé
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-200 dark:border-gray-700">
        <div className="flex flex-wrap gap-3">
          ...
          {/* Filtre département — masqué pour MANAGER (le backend a déjà filtré) */}
          {userRole !== 'MANAGER' && (
            <select
              value={departmentFilter}
              onChange={(e) => setDepartmentFilter(e.target.value)}
