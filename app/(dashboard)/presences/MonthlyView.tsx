...
export default function MonthlyView({ data, date, setDate, userRole, userDepartment }: MonthlyViewProps) {
  ...
  const StatusCell = ({ status, isToday }: { status: string; isToday: boolean }) => {
    ...
    return (
      <div
        className={`w-full h-full min-h-[32px] min-w-[24px] border-b border-r ${colors[status] ?? 'bg-gray-100'} ${isToday ? 'ring-2 ring-sky-500 ring-inset' : 'border-gray-100 dark:border-gray-800'}`}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex flex-wrap gap-4">
          {[
            ...
            { color: 'bg-orange-400',  label: 'Retard' },
            { color: 'bg-red-400',     label: 'Absent non justifié' },
            { color: 'bg-purple-400',  label: 'Télétravail' },
            { color: 'bg-sky-400',     label: 'Congé' },
            { color: 'bg-blue-400',    label: 'Jour férié' },
            { color: 'bg-gray-300 dark:bg-gray-700 border border-gray-400', label: 'Jour non ouvrable' },
            { color: 'bg-white dark:bg-gray-900 border-2 border-gray-300',  label: 'Jour futur' },
          ].map(({ color, label }) => (
                  ...
                  return (
                    <div key={d} className={`w-10 shrink-0 text-center p-2 border-r ${
                      ...
                      <div className={`text-[10px] font-bold ${
                        ...
                        : !isWorking ? 'text-gray-500 dark:text-gray-400'
                        : 'text-gray-400'
                      }`}>{dayName}</div>
                      <div className={`text-xs font-bold ${
                        isToday ? 'text-sky-600 dark:text-sky-400'
                        : !isWorking ? 'text-gray-500 dark:text-gray-400'
                        : isFuture || isBeforeCompanyCreation() ? 'text-gray-400'
