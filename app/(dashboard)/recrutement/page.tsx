...
import { 
  ...
  Trash2, Edit, BrainCircuit, User, Zap
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { api } from '@/services/api';
...
export default function RecruitmentPage() {
  ...
  const handleCopyLink = () => {
    if (!shareJob) return;
    const baseUrl = shareJob.processingMode === 'AI_ASSISTED' 
      ? `${window.location.origin}/jobs/ia/${shareJob.id}`
      : `${window.location.origin}/jobs/manuel/${shareJob.id}`;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative group">
            ...
            {/* Dropdown */}
            <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-white/10 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
              <Link 
          ...
          >
            ...
            >
              ...
              <div className="p-6 bg-black/30 rounded-2xl mb-8">
                ...
                <div className="flex items-center gap-3 bg-black/40 p-2 rounded-xl">
                  <div className="flex-1 px-3 font-mono text-sm text-cyan-400 truncate">
                    {shareJob.processingMode === 'AI_ASSISTED' 
                      ? `${window.location.origin}/jobs/ia/${shareJob.id}`
                      : `${window.location.origin}/jobs/manuel/${shareJob.id}`
                    }
                  </div>
              ...
              <div className="flex gap-4">
                <a 
                  href={shareJob.processingMode === 'AI_ASSISTED' ? `/jobs/ia/${shareJob.id}` : `/jobs/manuel/${shareJob.id}`}
                  target="_blank" 
                  className="flex-1 py-4 border border-white/10 hover:bg-white/5 rounded-2xl font-bold text-white flex justify-center items-center gap-2"
                >
                  <ExternalLink size={20} /> Voir
