'use client';
// ============================================================================
// 📁 app/(dashboard)/rh-blog/page.tsx — FIX BOUCLE INFINIE + 429
//
// PROBLÈME : user changeait de référence à chaque render
//   → load() se recréait → useEffect se redéclenchait → boucle → 429
//
// FIX : extraire userId + companyId en primitives stables dans useRef
// ============================================================================
...
function PostCard({ post, userId, onLike, onDelete }: {
  post: BlogPostSummary; userId: string;
            ...
            {isOwn && (
              <div className="relative">
                <button onClick={() => setMenu(v => !v)}
                  className="px-2 py-1 text-slate-500 hover:text-white text-xs rounded-lg hover:bg-white/5">···</button>
                {menu && (
                  <div className="absolute right-0 top-7 z-20 bg-[#0F1E35] border border-white/10 rounded-xl shadow-xl py-1 w-36"
                    ...
                    <Link href={`/rh-blog/${post.slug}/edit`}
                      className="flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-white/5 no-underline">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <p className="text-sm text-slate-400 text-center mb-6">
          "<span className="text-white font-semibold">{title}</span>" sera supprimé définitivement.
        </p>
        <div className="flex gap-3">
          <button onClick={onCancel}
            className="flex-1 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-sm font-semibold text-slate-300">
            Annuler
          </button>
...
export default function RhBlogPage() {
  ...
  // ✅ FIX PRINCIPAL : on lit user UNE seule fois au montage
  // authService.getCurrentUser() retourne un objet différent à chaque appel
  // → on stabilise les primitives dans des refs pour ne pas recréer load()
  ...
  useEffect(() => {
    ...
    if (userRole === 'SUPER_ADMIN') router.replace('/admin/blog');
  }, []); // ← intentionnellement vide, on vérifie juste au montage
  ...
  const canPost = CAN_POST.includes(userRole);
  ...
  // Quota — une seule fois
  useEffect(() => {
    if (!canPost) return;
    blogApi.quota().then(setQuota).catch(() => {});
  }, []); // ← intentionnellement vide

  // ✅ FIX : dépendances primitives (string) pas d'objet user
  const load = useCallback(async () => {
    setLoading(true);
    setApiError('');
    try {
      const params: Parameters<typeof blogApi.list>[0] = { page, limit: 9 };
      if (cat)                                               params.category   = cat;
      if (dSearch)                                           params.q          = dSearch;
      if (userCompanyId && userCompanyId !== 'undefined')   params.companyId  = userCompanyId;
  ...
  // ✅ userId/userRole/userCompanyId sont des strings stables — pas d'objet
  }, [page, cat, dSearch, userCompanyId]);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          {canPost && quota && !quota.unlimited && (
            <div className={`px-3 py-2 rounded-xl border text-center ${
              quotaFull ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'
            }`}>
      ...
      {quotaFull && (
        <div className="flex items-center gap-3 px-4 py-3 bg-red-500/8 border border-red-500/20 rounded-xl text-sm text-red-400">
          ...
          Quota mensuel atteint — {quota?.limit} posts/mois. Revenez le mois prochain.
        </div>
      )}
      ...
      {apiError && !loading && (
        <div className="flex items-start gap-3 px-4 py-4 bg-red-500/8 border border-red-500/20 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed font-mono">{apiError}</p>
            <button onClick={load}
