...
const CAT_COLOR: Record<string, string> = {
  ANNONCE:'#8B8F98', PAIE:'#10B981', DROIT_TRAVAIL:'#10B981',
  RECRUTEMENT:'#10B981', FORMATION:'#10B981', TEMOIGNAGE:'#10B981', GENERAL:'#8B8F98',
};
...
function PostCard({ post, onLike }: { post: BlogPostSummary; onLike: (slug: string) => void }) {
  ...
  const cc  = CAT_COLOR[post.category] || '#8B8F98';
  ...
  return (
    <div onMouseEnter={() => sH(true)} onMouseLeave={() => sH(false)}
      style={{ background: h ? '#111317' : '#0B0C0F', border: `1px solid ${h ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.08)'}`,
      ...
      {post.coverImage ? (
        <div style={{ height: 180, overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
          ...
          <div style={{ position:'absolute', inset:0, background:'linear-gradient(to bottom,transparent 40%,#0B0C0FE0)' }}/>
          <span style={{ position:'absolute', top:12, left:12, fontSize:10, fontWeight:600, letterSpacing:'0.07em',
          ...
          {isSA && <span style={{ position:'absolute', top:12, right:12, fontSize:10, fontWeight:600, color:'#FAFAFA',
            background:'linear-gradient(135deg,#10B981,#10B981)', padding:'3px 10px', borderRadius:99 }}>Officiel</span>}
        </div>
      ) : (
        <div style={{ height:56, flexShrink:0, background:`${cc}18`, display:'flex', alignItems:'center', padding:'0 18px', gap:8 }}>
          <span style={{ fontSize:11, fontWeight:600, color:cc, letterSpacing:'0.08em', textTransform:'uppercase' }}>{catLabel}</span>
          {isSA && <span style={{ marginLeft:'auto', fontSize:10, fontWeight:600, color:'#FAFAFA', background:'linear-gradient(135deg,#10B981,#10B981)', padding:'3px 10px', borderRadius:99 }}>Officiel</span>}
        </div>
      )}
      ...
      <div style={{ padding:'18px 20px 20px', display:'flex', flexDirection:'column', gap:10, flex:1 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ width:26, height:26, borderRadius:'50%', background:'linear-gradient(135deg,#10B981,#10B981)',
            display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:600, color:'#FAFAFA', flexShrink:0 }}>
          ...
          {company && !isSA && <span style={{ fontSize:11, color:'#8B8F98' }}>· {company}</span>}
        </div>

        <Link href={`/blog/${post.slug}`} style={{ textDecoration:'none' }}>
          <h3 style={{ fontSize:15.5, fontWeight:600, color:'#FAFAFA', letterSpacing:'-0.02em', lineHeight:1.35,
        ...
        {post.excerpt && (
          <p style={{ fontSize:13, color:'#8B8F98', lineHeight:1.65,
        ...
        <div style={{ marginTop:'auto', paddingTop:10, borderTop:'1px solid rgba(255,255,255,0.08)',
          ...
          <span style={{ fontSize:11.5, color:'#8B8F98' }}>{date}</span>
          <div style={{ display:'flex', alignItems:'center', gap:4 }}>
            ...
            <button onClick={() => onLike(post.slug)}
              style={{ display:'flex', alignItems:'center', gap:5, background:'none', border:'none', cursor:'pointer',
                color: post.hasLiked ? '#10B981' : '#8B8F98', fontSize:12, fontWeight:600, fontFamily:'inherit',
            ...
            <button onClick={share}
              style={{ display:'flex', alignItems:'center', background:'none', border:'none', cursor:'pointer',
                color:'#8B8F98', padding:'4px 8px', borderRadius:7, transition:'color 0.15s' }}
            ...
            <Link href={`/blog/${post.slug}`}
              style={{ fontSize:12, fontWeight:700, color:'#10B981', textDecoration:'none', padding:'4px 8px' }}>
              Lire →
            </Link>
          </div>
        </div>
...
export default function BlogPageLegacy()  {
  ...
  return (
    <div style={{ background:'#050607', minHeight:'100vh', fontFamily:"system-ui,-apple-system,'Segoe UI',sans-serif", color:'#FAFAFA', overflowX:'hidden' }}>
      <GridBg/>
      <Navbar/>
      ...
      <section style={{ position:'relative', padding:'150px 32px 70px', overflow:'hidden', zIndex:1 }}>
        <div style={{ position:'absolute', top:-100, left:'50%', transform:'translateX(-50%)', width:600, height:400,
          background:'#10B981', borderRadius:'50%', filter:'blur(120px)', opacity:0.08, pointerEvents:'none' }}/>
        <div style={{ maxWidth:1280, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'flex-end', flexWrap:'wrap', gap:24 }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'6px 14px', background:'rgba(255,255,255,0.03)',
              border:'1px solid rgba(255,255,255,0.1)', borderRadius:99, fontSize:13, color:'#8B8F98',
              marginBottom:18 }}>// Blog RH</div>
            <h1 style={{ fontSize:'clamp(26px,5vw,52px)', fontWeight:600, letterSpacing:'-0.03em', lineHeight:1.1, marginBottom:14, color:'#FAFAFA' }}>
              Partages & actualités<br/>
              <span style={{ color:'#10B981' }}>
                de la communauté RH.
              </span>
            </h1>
            <p style={{ fontSize:16, color:'#8B8F98', lineHeight:1.7, maxWidth:480 }}>
              Bonnes pratiques, actualités fiscales, retours d'expérience — partagés par les RH du Congo.
            </p>
          </div>
          <Link href="/auth/login" style={{ display:'inline-flex', alignItems:'center', gap:7, border:'1px solid rgba(255,255,255,0.08)',
            color:'#8B8F98', textDecoration:'none', fontWeight:600, fontSize:13.5, padding:'10px 18px', borderRadius:10 }}>
      ...
      {/* Filtres */}
      <section style={{ position:'relative', zIndex:1, borderTop:'1px solid rgba(255,255,255,0.08)', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'rgba(255,255,255,0.01)' }}>
            ...
            {CATS.map(c => (
              <button key={c.value} onClick={() => setCat(c.value)}
                style={{ padding:'6px 13px', borderRadius:99, fontSize:12.5, fontWeight:600, cursor:'pointer', fontFamily:'inherit',
                  border:`1px solid ${cat === c.value ? '#10B981' : 'rgba(255,255,255,0.08)'}`,
                  background: cat === c.value ? 'rgba(16,185,129,0.1)' : 'transparent',
                  color: cat === c.value ? '#10B981' : '#8B8F98', transition:'all 0.15s' }}>
                {c.label}
              </button>
            ))}
          </div>
          <div style={{ position:'relative' }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8B8F98" strokeWidth="2"
              style={{ position:'absolute', left:11, top:'50%', transform:'translateY(-50%)', pointerEvents:'none' }}>
              <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
            </svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher..."
              style={{ padding:'8px 13px 8px 33px', background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.08)',
                borderRadius:9, color:'#FAFAFA', fontSize:13, outline:'none', fontFamily:'inherit', width:200,
                transition:'border-color 0.2s' }}
              onFocus={e => e.target.style.borderColor='#10B981'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.08)'}/>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth:1280, margin:'0 auto', padding:'56px 32px' }}>
          {loading ? (
            <div style={{ textAlign:'center', padding:'72px 0' }}>
              ...
              <p style={{ color:'#8B8F98', fontSize:14 }}>Chargement...</p>
            </div>
          ) : posts.length === 0 ? (
            <div style={{ textAlign:'center', padding:'72px 0' }}>
              <div style={{ fontSize:44, marginBottom:14 }}></div>
              <h3 style={{ fontSize:19, fontWeight:700, color:'#FAFAFA', marginBottom:8 }}>Aucun article</h3>
              <p style={{ color:'#8B8F98', fontSize:14 }}>{dSearch ? `Aucun résultat pour "${dSearch}"` : 'Pas encore d\'articles.'}</p>
            </div>
          ) : (
            <>
              {/* Featured */}
              {featured && !dSearch && page === 1 && (
                <div style={{ marginBottom:36 }}>
                  <p style={{ fontSize:11, fontWeight:700, color:'#8B8F98', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:14 }}>À la une</p>
                  <div style={{ background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.08)', borderRadius:20, overflow:'hidden',
                    ...
                    <div style={{ position:'relative', overflow:'hidden', minHeight:220 }}>
                      {featured.coverImage ? (
                        ...
                        <div style={{ position:'absolute', inset:0, background:`linear-gradient(135deg,${CAT_COLOR[featured.category] || '#8B8F98'}18,#0B0C0F)` }}/>
                      )}
                      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to right,transparent 40%,#0B0C0FCC)' }}/>
                    </div>
                    <div style={{ padding:'32px 28px', display:'flex', flexDirection:'column', justifyContent:'center', gap:14 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <div style={{ width:28, height:28, borderRadius:'50%', background:'linear-gradient(135deg,#10B981,#10B981)',
                          display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:600, color:'#FAFAFA' }}>
                        ...
                        <span style={{ fontSize:11, color:'#8B8F98' }}>· {new Date(featured.publishedAt).toLocaleDateString('fr-FR',{day:'numeric',month:'short'})}</span>
                      </div>
                      <Link href={`/blog/${featured.slug}`} style={{ textDecoration:'none' }}>
                        <h2 style={{ fontSize:'clamp(17px,2.5vw,24px)', fontWeight:600, color:'#FAFAFA', letterSpacing:'-0.025em', lineHeight:1.25, cursor:'pointer' }}>
                          {featured.title}
                        </h2>
                      </Link>
                      {featured.excerpt && <p style={{ fontSize:14, color:'#8B8F98', lineHeight:1.7, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{featured.excerpt}</p>}
                      <div style={{ display:'flex', gap:8 }}>
                        <Link href={`/blog/${featured.slug}`} style={{ display:'inline-flex', alignItems:'center', gap:6, background:'linear-gradient(135deg,#10B981,#10B981)', color:'#FAFAFA', textDecoration:'none', fontWeight:600, fontSize:13, padding:'9px 18px', borderRadius:9 }}>
                        ...
                        <button onClick={() => handleLike(featured.slug)}
                          style={{ display:'flex', alignItems:'center', gap:5, background: featured.hasLiked ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.05)',
                            border:`1px solid ${featured.hasLiked ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.1)'}`,
                            borderRadius:9, padding:'8px 14px', cursor:'pointer', color: featured.hasLiked ? '#10B981' : '#8B8F98',
              ...
              {(dSearch || page > 1 ? posts : rest).length > 0 && (
                <>
                  <p style={{ fontSize:11, fontWeight:700, color:'#8B8F98', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:18 }}>
              ...
              {totalPgs > 1 && (
                <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:8, marginTop:44 }}>
                  <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1}
                    style={{ padding:'8px 15px', background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8,
                      color: page === 1 ? '#8B8F98' : '#FAFAFA', cursor: page === 1 ? 'not-allowed' : 'pointer', fontFamily:'inherit', fontSize:13, fontWeight:600 }}>
                    ← Préc.
                  </button>
                    ...
                    return (
                      <button key={pg} onClick={() => setPage(pg)}
                        style={{ width:36, height:36, borderRadius:8, fontFamily:'inherit',
                          border:`1px solid ${pg === page ? '#10B981' : 'rgba(255,255,255,0.08)'}`,
                          background: pg === page ? 'rgba(16,185,129,0.12)' : 'transparent',
                          color: pg === page ? '#10B981' : '#8B8F98', cursor:'pointer', fontWeight:700, fontSize:13 }}>
                        {pg}
                      </button>
                    );
                  })}
                  <button onClick={() => setPage(p => Math.min(totalPgs, p+1))} disabled={page === totalPgs}
                    style={{ padding:'8px 15px', background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8,
                      color: page === totalPgs ? '#8B8F98' : '#FAFAFA', cursor: page === totalPgs ? 'not-allowed' : 'pointer', fontFamily:'inherit', fontSize:13, fontWeight:600 }}>
                    Suiv. →
                  </button>
                </div>
              )}
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}
        ::-webkit-scrollbar{width:6px;background:#050607}::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.08);border-radius:3px}
