...
const CAT_COLOR: Record<string, string> = {
  ANNONCE:'#F59E0B', PAIE:'#2DD4BF', DROIT_TRAVAIL:'#64748B',
  RECRUTEMENT:'#10B981', FORMATION:'#059669', TEMOIGNAGE:'#EC4899', GENERAL:'#94A3B8',
};
...
function PostCard({ post, onLike }: { post: BlogPostSummary; onLike: (slug: string) => void }) {
  ...
  return (
    <div onMouseEnter={() => sH(true)} onMouseLeave={() => sH(false)}
      style={{ background: h ? '#111317' : '#0B0C0F', border: `1px solid ${h ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.07)'}`,
      ...
      {post.coverImage ? (
        <div style={{ height: 180, overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
          ...
          {isSA && <span style={{ position:'absolute', top:12, right:12, fontSize:10, fontWeight:800, color:'#fff',
            background:'linear-gradient(135deg,#10B981,#059669)', padding:'3px 10px', borderRadius:99 }}>Officiel</span>}
        </div>
      ) : (
        <div style={{ height:56, flexShrink:0, background:'#111317', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', padding:'0 18px', gap:8 }}>
          <span style={{ fontSize:11, fontWeight:800, color:cc, letterSpacing:'0.08em', textTransform:'uppercase', border:`1px solid ${cc}40`, borderRadius:99, padding:'3px 10px' }}>{catLabel}</span>
          {isSA && <span style={{ marginLeft:'auto', fontSize:10, fontWeight:800, color:'#fff', background:'linear-gradient(135deg,#10B981,#059669)', padding:'3px 10px', borderRadius:99 }}>Officiel</span>}
        </div>
      )}
      ...
      <div style={{ padding:'18px 20px 20px', display:'flex', flexDirection:'column', gap:10, flex:1 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ width:26, height:26, borderRadius:'50%', background:'linear-gradient(135deg,#10B981,#059669)',
...
export default function BlogPageLegacy()  {
  ...
  return (
    <div style={{ background:'#050607', minHeight:'100vh', fontFamily:"system-ui,-apple-system,'Segoe UI',sans-serif", color:'#F8FAFC', overflowX:'hidden' }}>
      <Navbar/>
      ...
      <section style={{ position:'relative', padding:'150px 32px 70px', overflow:'hidden', zIndex:1 }}>
        <div style={{ position:'absolute', top:-100, left:'50%', transform:'translateX(-50%)', width:600, height:400,
          background:'#059669', borderRadius:'50%', filter:'blur(120px)', opacity:0.08, pointerEvents:'none' }}/>
        <div style={{ maxWidth:1280, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'flex-end', flexWrap:'wrap', gap:24 }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'6px 16px', background:'rgba(16,185,129,0.08)',
              border:'1px solid rgba(16,185,129,0.2)', borderRadius:99, fontSize:12, fontWeight:700, color:'#10B981',
        ...
        <div style={{ maxWidth:1280, margin:'0 auto', padding:'14px 32px', display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', justifyContent:'space-between' }}>
            ...
            {CATS.map(c => (
              <button key={c.value} onClick={() => setCat(c.value)}
                style={{ padding:'6px 13px', borderRadius:99, fontSize:12.5, fontWeight:600, cursor:'pointer', fontFamily:'inherit',
                  border:`1px solid ${cat === c.value ? '#10B981' : 'rgba(255,255,255,0.07)'}`,
                  background: cat === c.value ? 'rgba(16,185,129,0.1)' : 'transparent',
                  color: cat === c.value ? '#10B981' : '#94A3B8', transition:'all 0.15s' }}>
                {c.label}
              </button>
            ))}
          </div>
          <div style={{ position:'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher..."
              style={{ padding:'8px 13px 8px 33px', background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.07)',
                borderRadius:9, color:'#F8FAFC', fontSize:13, outline:'none', fontFamily:'inherit', width:200,
                transition:'border-color 0.2s' }}
              onFocus={e => e.target.style.borderColor='#10B981'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.07)'}/>
          </div>
        </div>
      </section>
            ...
            <>
              ...
              {featured && !dSearch && page === 1 && (
                <div style={{ marginBottom:36 }}>
                  ...
                  <div style={{ background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.07)', borderRadius:20, overflow:'hidden',
                    ...
                    <div style={{ position:'relative', overflow:'hidden', minHeight:220 }}>
                      {featured.coverImage ? (
                        ...
                        <div style={{ position:'absolute', inset:0, background:`linear-gradient(135deg,${CAT_COLOR[featured.category] || '#94A3B8'}18,#0B0C0F)` }}/>
                      )}
                    ...
                    <div style={{ padding:'32px 28px', display:'flex', flexDirection:'column', justifyContent:'center', gap:14 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <div style={{ width:28, height:28, borderRadius:'50%', background:'linear-gradient(135deg,#10B981,#059669)',
                      ...
                      <div style={{ display:'flex', gap:8 }}>
                        <Link href={`/blog/${featured.slug}`} style={{ display:'inline-flex', alignItems:'center', gap:6, background:'#FAFAFA', color:'#000', textDecoration:'none', fontWeight:600, fontSize:13, padding:'9px 18px', borderRadius:9 }}>
              ...
              {totalPgs > 1 && (
                <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:8, marginTop:44 }}>
                  <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1}
                    style={{ padding:'8px 15px', background:'#0B0C0F', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8,
                    ...
                    return (
                      <button key={pg} onClick={() => setPage(pg)}
                        style={{ width:36, height:36, borderRadius:8, fontFamily:'inherit',
                          border:`1px solid ${pg === page ? '#10B981' : 'rgba(255,255,255,0.07)'}`,
                          background: pg === page ? 'rgba(16,185,129,0.12)' : 'transparent',
                          color: pg === page ? '#10B981' : '#94A3B8', cursor:'pointer', fontWeight:700, fontSize:13 }}>
                        {pg}
                      </button>
                    );
                  })}
