...
export default function CompaniesPage() {
  ...
  const fetchCompanies = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/public/companies`);
  ...
  const filtered = companies.filter(c => {
    ...
    const matchSearch = !search || c.legalName.toLowerCase().includes(s) || c.industry?.toLowerCase().includes(s);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24">
            ...
            {hasFilters && (
              <button
                onClick={() => { setSearch(''); setSelectedIndustry(''); }}
                ...
                {paginated.map((company, i) => {
                  ...
                  return (
                    ...
                    >
                      <Link
                        href={href}
                        className="group flex flex-col bg-slate-900 border border-slate-800 hover:border-slate-600 rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/40"
                      >
                        {/* Banner */}
                        <div className="relative h-28 overflow-hidden bg-slate-800">
                          {company.careerPageBanner ? (
                            ...
                            <div
                              ...
                              style={{ background: `linear-gradient(135deg, ${accent}25, ${accent}08, transparent)` }}
                            />
                          )}
                        ...
                        <div className="px-5 -mt-7 mb-3">
                          <div className="w-14 h-14 rounded-xl bg-slate-900 border-2 border-slate-950 shadow-xl flex items-center justify-center overflow-hidden">
                            {logo ? (
                              <Image src={logo} alt={company.legalName} width={56} height={56} className="object-contain" />
                            ) : (
                        ...
                        <div className="px-5 pb-5 flex-1 flex flex-col">
                          <h3 className="font-bold text-white group-hover:text-cyan-400 transition-colors text-base mb-1 leading-snug">
                            {company.legalName}
                          </h3>
                          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 mb-3">
                            ...
                            {company.city && (
                              <>
                                {company.industry && <span className="text-slate-700">·</span>}
                                <span className="flex items-center gap-1">
                          ...
                          <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-800">
                            <span
                              ...
                              style={{ color: company.jobCount > 0 ? accent : '#475569' }}
                            >
                              {company.jobCount > 0
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-12">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-2.5 rounded-xl border border-slate-800 text-slate-500 hover:text-white hover:border-slate-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-sm flex items-center gap-1.5"
                >
                  ← Précédent
                </button>
