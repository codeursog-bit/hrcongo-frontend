...
// ✅ Layout PME white-label — identique à l'original
// 🆕 Ajoute OnboardingChecklist uniquement pour le rôle ADMIN
...
export default function PmeLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Stocker le rôle pour conditionner l'onboarding
  ...
  return (
    <DashboardShell
      ...
      skipAuthCheck
    >
      {/* Onboarding uniquement pour ADMIN — sans étape paie (gérée par le cabinet) */}
      {userRole === 'ADMIN' && (
        <OnboardingChecklist hidePaie />
      )}
      {children}
    </DashboardShell>
  );
