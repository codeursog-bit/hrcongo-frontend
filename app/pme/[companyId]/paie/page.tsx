    ...
    <div className="space-y-6 min-h-screen pb-32" onClick={() => setShowMonthSelector(false)}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <PayrollMonthSelector
          ...
          onMonthChange={(m: string) => { setSelectedMonth(m); setShowMonthSelector(false); setCurrentPage(1); }}
          onYearChange={(y: number) => { setSelectedYear(y); setCurrentPage(1); }}
          isOpen={showMonthSelector}
          onToggle={(e: React.MouseEvent) => { e.stopPropagation(); setShowMonthSelector(!showMonthSelector); }}
        />
              ...
              {paginatedEntries.length === 0 ? (
                ...
                paginatedEntries.map(entry => (
                  <PayrollTableRow
                    ...
                    onStatusChange={(status: string) => handleStatusChange(entry.id, status)}
