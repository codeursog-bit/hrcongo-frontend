...
const fmtD = (d?: string) => d
  ...
  : '—';
const mois = (h: string, e: string) =>
  Math.floor((new Date(e).getTime() - new Date(h).getTime()) / (1000 * 60 * 60 * 24 * 30.44));
...
function Bdg({ c, v = 'd' }: { c: React.ReactNode; v?: 'd' | 's' | 'w' | 'r' | 'i' }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${m}`}>
      {c}
    </span>
  );
}
...
// ─── Sélecteur de nature de rupture ──────────────────────────────────────────
...
// ─── Page principale ──────────────────────────────────────────────────────────
// La convention est gérée exclusivement par ConventionGuard (parent).
// Cette page consomme uniquement le statut via useConvention().

export default function ContractRupturePage() {
  // Convention — lecture seule depuis le contexte du Guard
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100">
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Seuil de 5 licenciements économiques sur 30 jours atteint.
              </p>
            ...
            <button onClick={() => setPse(false)} className="text-slate-500 hover:text-slate-300 text-xl leading-none">
              &times;
            </button>
          </div>
        </div>
      ...
      <div className="sticky top-0 z-30 bg-[#020617]/90 backdrop-blur-xl border-b border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-3">
          ...
          {/* Badge convention — lu depuis le contexte, pas recalculé ici */}
          ...
          <Link
            href="/pse"
            className="hidden sm:flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-200 border border-white/10 hover:border-white/20 px-2.5 py-1.5 rounded-lg transition-all shrink-0"
          >
        ...
        {tab === 'historique' && (
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                onClick={loadHist}
                className="text-xs font-semibold text-slate-500 hover:text-slate-200 border border-white/10 px-3 py-1.5 rounded-lg transition-all"
              >
                Actualiser
              </button>
            </div>
              ...
              : (
                <div className="space-y-2">
                  {hist.map((r: any) => (
                    <div
                      key={r.id}
                      className="bg-slate-900/60 border border-white/10 rounded-xl p-4 flex items-center gap-4 hover:border-white/20 transition-all"
                    >
                      <div className="flex-1 min-w-0">
        ...
        {tab === 'nouveau' && step === 1 && (
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5">
                ...
                {empLoad
                  ...
                  ? (
                    <p className="text-center py-8 text-slate-600 text-xs">
                      {employees.length === 0 ? 'Aucun employé actif trouvé' : 'Aucun résultat'}
                    </p>
                  )
                  : (
                            ...
                            {isSel && form.ruptureDate && (
                              <div className="mt-2 pt-2 border-t border-cyan-500/20 text-xs text-cyan-400 font-medium">
                                ...
                                {e.professionalCategory && (
                                  <span className="text-slate-500 ml-2">· {e.professionalCategory}</span>
                                )}
                              </div>
                            )}
                          </button>
            ...
            <div className="space-y-4">
                ...
                <>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={form.congesPrisAnneeEnCours}
                  ...
                  <div className="flex flex-wrap gap-4">
                    <Chk
                      ...
                      onChange={() => setForm(f => ({ ...f, noticeWorked: !f.noticeWorked }))}
                      label="Préavis travaillé"
                    />
                    <Chk
                      checked={form.noticeWaived}
                      onChange={() => setForm(f => ({ ...f, noticeWaived: !f.noticeWaived }))}
                      label="Préavis dispensé"
                    />
                    <Chk
                      checked={form.employerInitiated}
                      onChange={() => setForm(f => ({ ...f, employerInitiated: !f.employerInitiated }))}
                      label="Initiative employeur"
                    />
                  </div>
                </>
              } />
              ...
              <button
                onClick={doCalc}
                disabled={!emp || !form.ruptureType || calcLoad}
                className={`w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                  emp && form.ruptureType && !calcLoad
          ...
          <div ref={calcRef} className="space-y-5">
                ...
                {calc.eligibilite.raisons.map((r, i) => (
                  <span
                    ...
                    className={`text-xs font-medium ${
                      r.startsWith('✅') ? 'text-emerald-400' : r.startsWith('❌') ? 'text-red-400' : 'text-slate-500'
                    }`}
                  >
                    {r}
                  </span>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5">
              ...
              <Crd t="Décompte — Solde de tout compte" ch={
                <div className="space-y-2">
                  {decompteLines.map((r, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between gap-3 p-3 bg-slate-800/40 rounded-xl border border-white/5"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-200 leading-snug">{r.l}</p>
                        {'sub' in r && r.sub && (
                          <p className="text-xs text-slate-600 mt-0.5 truncate">{r.sub as string}</p>
                        )}
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-3 border-t border-white/10">
                    <div className="p-3 bg-slate-800/60 rounded-xl">
                      ...
                      <p className="text-lg font-bold">
                        {fmt(calc.totaux.brutImposable)} <span className="text-xs text-slate-600">FCFA</span>
                      </p>
                      {calc.totaux.its > 0 && (
                        <p className="text-xs text-red-400 mt-0.5">ITS −{fmt(calc.totaux.its)} FCFA</p>
                      )}
                    </div>
                    <div className="p-3 bg-gradient-to-br from-cyan-500/10 to-blue-600/10 border border-cyan-500/20 rounded-xl">
                      ...
                      <p className="text-xl font-black text-cyan-400">
                        {fmt(calc.totaux.net)} <span className="text-xs text-cyan-700">FCFA</span>
                      </p>
                    </div>
                  </div>
                  <p className="text-xs text-slate-700 text-right">
                    Brut total : {fmt(bt)} · Exonéré : {fmt(calc.totaux.indemnitesExonerees)} FCFA
                  </p>
                </div>
              } />
              ...
              <div className="space-y-4">
                    ...
                    {[
                      ...
                      <div key={r.l} className="mb-3 last:mb-0">
                        ...
                        <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${r.c} rounded-full transition-all duration-700`}
                            style={{ width: `${pct(r.v, bt)}%` }}
                          />
                        </div>
                      </div>
                    ))}
                ...
                <Link
                  href="/pse"
                  className="flex items-center justify-between p-4 bg-slate-900/60 border border-white/10 hover:border-purple-500/30 rounded-xl transition-all group"
                >
                  <div>
                    <p className="text-xs font-bold text-slate-400 group-hover:text-purple-300">
            ...
            {/* Documents */}
            <Crd
              t="Documents officiels"
              s={rid ? undefined : 'Disponibles après confirmation'}
              ch={
                <>
                    ...
                    <div className="rounded-xl overflow-hidden border border-white/10">
                      <div className="flex items-center justify-between px-4 py-2 bg-slate-800/60 border-b border-white/10">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => { setActiveDoc(null); setDocHtml(null); }}
