...
//
// Guard + Modal de sélection convention.
// S'intègre dans la page rupture sans modifier aucun autre composant.
//
// Usage :
//   import { ConventionGuard } from '@/components/ConventionGuard';
//   <ConventionGuard><ContractRupturePage /></ConventionGuard>
// ============================================================================

import { useState } from 'react';
import { useConventionGuard, PredefinedConvention } from '@/hooks/useConventionGuard';
...
function ConventionSelectModal({
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#0d1117', borderRadius: 20, border: '1px solid #1e293b',
        padding: '36px 32px', maxWidth: 700, width: '100%',
          ...
          {predefined.map(c => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 12, fontWeight: 600, color: isSel ? '#e2e8f0' : '#64748b', lineHeight: 1.4 }}>
        ...
        <button
          ...
          style={{
            ...
            transition: 'all .3s', opacity: activating ? .7 : 1,
          }}
        >
          {activating
            ? '⏳ Activation en cours…'
        ...
        <p style={{ textAlign: 'center', fontSize: 11, color: '#334155', margin: '12px 0 0', lineHeight: 1.5 }}>
...
function ConventionBadge({ code, name }: { code: string; name: string }) {
  return (
    <div style={{
      ...
      <span style={{
        padding: '2px 10px', borderRadius: 20, fontWeight: 700, letterSpacing: .5,
        background: '#1e1b4b', color: '#a5b4fc', border: '1px solid #6366f125',
        fontSize: 11, textTransform: 'uppercase',
      }}>{code}</span>
      <span style={{ color: '#64748b' }}>
        {name.replace(' — République du Congo', '').replace(' — Congo', '').replace(' - ' + code, '')}
      </span>
    </div>
  );
}
...
export function ConventionGuard({ children }: { children: React.ReactNode }) {
  const {
    status, predefined, loading, showModal,
    activating, error, activateConvention,
  } = useConventionGuard();
  ...
  if (loading) {
    return (
      <div style={{
        ...
        background: '#0a0e1a', color: '#475569', fontSize: 14, gap: 10,
      }}>
  ...
  return (
    <>
      {/* Modal bloquant si pas de convention */}
      {showModal && (
      ...
      {/* Badge discret en haut si convention active */}
