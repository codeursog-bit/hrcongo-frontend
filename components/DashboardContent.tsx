...
import {
  ...
  Timer, Umbrella, Activity, Shield, FileText, Ticket
} from 'lucide-react';
import {
...
export const DashboardContent = () => {
  ...
  useEffect(() => {
    const handleEmployeeDeleted = (e: Event) => {
      ...
      // Met à jour totalEmployees dans stats (vue ADMIN/HR)
      setStats((prev: any) => ({
        ...prev,
        totalEmployees: Math.max(0, (prev.totalEmployees || 0) - 1),
      }));

      // Met à jour teamSize dans managerStats (vue MANAGER)
      // Vérifie que l'employé supprimé appartient bien au même département
  ...
  // ══════════════════════════════════════════════════════════════
  // 🆕 SYNC TEMPS RÉEL — écoute l'event d'ajout d'employé
  //    émis depuis la page de création (si vous l'ajoutez plus tard)
  // ══════════════════════════════════════════════════════════════
  useEffect(() => {
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="p-2 bg-sky-500/10 rounded-xl"><Shield size={20} className="text-sky-500" /></div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Tableau de bord Manager</h2>
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={() => router.push('/presences')} className="px-4 py-2 bg-sky-500 hover:bg-sky-600 text-white font-bold rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-sky-500/20 text-sm">
              <Activity size={16} /> Présences équipe
            </button>
            <button onClick={() => router.push('/conges')} className="px-4 py-2 bg-white dark:bg-slate-900/60 border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 text-gray-700 dark:text-white font-bold rounded-xl transition-all flex items-center gap-2 text-sm backdrop-blur-xl">
              <Calendar size={16} /> Congés
              {m.pendingLeaves > 0 && <span className="bg-orange-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{m.pendingLeaves}</span>}
            </button>
          </div>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <motion.div variants={itemVariants}><StatCard label="Membres Équipe" value={(m.teamSize || 0).toString()} trend={m.departmentName || 'Département'} isPositive={true} icon={Users} gradientFrom="from-cyan-500" gradientTo="to-blue-600" /></motion.div>
          <motion.div variants={itemVariants}><StatCard label="Présents" value={(m.presentCount || 0).toString()} trend={`${presenceRate}% de l'équipe`} isPositive={presenceRate >= 70} icon={UserCheck} gradientFrom="from-emerald-400" gradientTo="to-teal-600" /></motion.div>
          <motion.div variants={itemVariants}><StatCard label="Absents" value={(m.absentCount || 0).toString()} trend={m.lateCount > 0 ? `dont ${m.lateCount} retard${m.lateCount > 1 ? 's' : ''}` : "Aujourd'hui"} isPositive={m.absentCount === 0} icon={UserX} gradientFrom="from-orange-400" gradientTo="to-red-500" /></motion.div>
          <motion.div variants={itemVariants}><StatCard label="Congés en cours" value={(m.onLeaveCount || 0).toString()} trend={`${m.pendingLeaves || 0} en attente`} isPositive={m.pendingLeaves === 0} icon={Umbrella} gradientFrom="from-violet-500" gradientTo="to-purple-600" /></motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl overflow-hidden">
            <div className="p-5 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
              <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><UserX size={18} className="text-red-400" /> Absents du jour</h3>
            ...
            <div className="divide-y divide-gray-100 dark:divide-white/5 max-h-72 overflow-y-auto">
              {(m.absentMembers || []).length === 0 ? (
                <div className="p-8 text-center"><CheckCircle size={32} className="mx-auto text-emerald-400 mb-2" /><p className="text-sm font-bold text-gray-700 dark:text-slate-300">Tout le monde est là !</p></div>
          ...
          <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl overflow-hidden">
            <div className="p-5 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
              <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><Users size={18} className="text-sky-400" /> Mon équipe</h3>
              <button onClick={() => router.push('/employes')} className="text-xs text-sky-500 font-bold hover:text-sky-400 flex items-center gap-1 transition-colors">Voir tout <ChevronRight size={14} /></button>
            </div>
            <div className="divide-y divide-gray-100 dark:divide-white/5 max-h-72 overflow-y-auto">
              {(m.teamMembers || []).slice(0, 8).map((emp: any) => (
                <div key={emp.id} className="flex items-center gap-3 p-4 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  <img src={emp.photoUrl || `https://ui-avatars.com/api/?name=${emp.firstName}+${emp.lastName}&background=random`} className="w-9 h-9 rounded-full object-cover shrink-0 border border-gray-200 dark:border-white/10" alt="" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{emp.firstName} {emp.lastName}</p>
                    <p className="text-xs text-gray-500 dark:text-slate-400 truncate">{emp.position}</p>
                  </div>
                  <span className="text-[10px] bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-slate-400 px-2 py-0.5 rounded font-mono border border-gray-200 dark:border-white/10">{emp.contractType}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            <div className="bg-white dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl overflow-hidden">
              <div className="p-4 border-b border-gray-100 dark:border-white/5 flex items-center justify-between">
                <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><Calendar size={16} className="text-orange-400" /> Congés récents</h3>
                {m.pendingLeaves > 0 && <span className="text-xs bg-orange-500/10 text-orange-500 px-2 py-0.5 rounded-full font-bold border border-orange-500/20">{m.pendingLeaves} en attente</span>}
              </div>
                ...
                {(m.recentLeaveRequests || []).length === 0
                  ...
                  : m.recentLeaveRequests.map((l: any) => (
                    <div key={l.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold border ${l.status === 'PENDING' ? 'bg-orange-500/10 text-orange-500 border-orange-500/20' : l.status === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
            ...
            <div className="bg-white dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-gray-200 dark:border-white/10 shadow-xl overflow-hidden">
              <div className="p-4 border-b border-gray-100 dark:border-white/5">
                <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2"><Timer size={16} className="text-emerald-400" /> Derniers pointages</h3>
              </div>
              <div className="divide-y divide-gray-100 dark:divide-white/5 max-h-40 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div onClick={() => router.push('/presences/pointage')} className="cursor-pointer bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 rounded-2xl p-4 border border-gray-100 dark:border-white/5 transition-all hover:-translate-y-0.5 relative overflow-hidden">
              ...
              {myStats.checkIn ? (
                <>
                  ...
                  <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-1">Entrée {new Date(myStats.checkIn).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                </>
            ...
            <div className="bg-gray-50 dark:bg-white/5 rounded-2xl p-4 border border-gray-100 dark:border-white/5">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => router.push('/conges/nouveau')} className="w-full text-[11px] bg-sky-500/10 hover:bg-sky-500/20 text-sky-600 dark:text-sky-400 px-2 py-1.5 rounded-lg font-bold transition-colors border border-sky-500/20 text-left flex items-center gap-1.5"><Calendar size={12} /> + Congé</button>
                <button onClick={() => router.push('/presences/pointage-manuel')} className="w-full text-[11px] bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-2 py-1.5 rounded-lg font-bold transition-colors border border-emerald-500/20 text-left flex items-center gap-1.5"><Fingerprint size={12} /> Pointer équipe</button>
              </div>
            </div>
          </div>
        </motion.div>
  ...
  if (userRole === 'EMPLOYEE') {
    return (
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <motion.div variants={itemVariants}><StatCard label="Congés Posés" value={myStats.pendingLeaves?.toString() || '0'} trend="En attente" isPositive={true} icon={Calendar} gradientFrom="from-orange-400" gradientTo="to-red-500" /></motion.div>
          <motion.div variants={itemVariants}><StatCard label="Solde Congés" value={`${myStats.remainingLeaves || '0'} j`} trend="Disponibles" isPositive={true} icon={Clock} gradientFrom="from-emerald-400" gradientTo="to-teal-600" /></motion.div>
          <motion.div variants={itemVariants}><StatCard label="Dernier Salaire" value={myStats.lastSalary || '0 F'} trend={myStats.lastSalaryMonth || '-'} isPositive={true} icon={Wallet} gradientFrom="from-blue-500" gradientTo="to-indigo-600" isPrivate={true} showValue={showSalary} onToggleVisibility={() => setShowSalary(!showSalary)} /></motion.div>
  ...
  // ══════════════════════════════════════════════════════════════
  // 🔑 VUE ADMIN / HR
  // 🆕 Carte "Congés en attente" → "Demandes en attente" (total combiné
  //    congés + absences + permissions, calculé côté backend)
  // ══════════════════════════════════════════════════════════════
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <motion.div variants={itemVariants}><StatCard label="Demandes en attente" value={(stats.pendingRequestsTotal ?? ((stats.pendingLeaves||0)+(stats.pendingAbsences||0)+(stats.pendingPermissions||0))).toString()} trend="À traiter" isPositive={(stats.pendingRequestsTotal ?? ((stats.pendingLeaves||0)+(stats.pendingAbsences||0)+(stats.pendingPermissions||0))) === 0} icon={Calendar} gradientFrom="from-orange-400" gradientTo="to-red-500" /></motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} onClick={() => router.push(item.action)} className="group cursor-pointer bg-white dark:bg-slate-900/60 backdrop-blur-xl p-5 rounded-2xl border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20 transition-all hover:-translate-y-1 relative overflow-hidden shadow-sm">
                <div className={`absolute top-0 right-0 w-20 h-20 opacity-10 rounded-bl-full ${item.color}`} />
          ...
          <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 border border-gray-200 dark:border-white/10 shadow-xl">
            ...
            <div className="space-y-4">
              {stats.recentPayrolls?.length > 0 ? stats.recentPayrolls.map((payroll: any) => (
                <div key={payroll.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                  <div className="w-10 h-10 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-500 flex items-center justify-center border border-emerald-500/30 font-bold text-xs">{payroll.employee.firstName[0]}{payroll.employee.lastName[0]}</div>
                  <div className="flex-1 min-w-0">
            ...
            <button onClick={() => router.push('/paie')} className="w-full mt-4 py-2 text-sm font-bold text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors border border-dashed border-gray-300 dark:border-slate-700 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5">Voir tout l'historique</button>
          </motion.div>
        </div>
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 border border-gray-200 dark:border-white/10 shadow-xl flex flex-col">
          ...
          <div className="flex-1 space-y-6 overflow-y-auto max-h-[400px] pr-2">
            {stats.recentActivities?.length > 0 ? stats.recentActivities.map((act: any, i: number) => {
              ...
              if (act.type === 'ABSENCE')    { icon = <FileText size={16} />; color = 'bg-orange-100 text-orange-600 dark:bg-orange-900/20 dark:text-orange-400'; }
              if (act.type === 'PERMISSION') { icon = <Ticket size={16} />;   color = 'bg-violet-100 text-violet-600 dark:bg-violet-900/20 dark:text-violet-400'; }
              ...
              return (
                <div key={act.id} className="flex gap-4 relative">
                  {i !== stats.recentActivities.length - 1 && <div className="absolute left-[15px] top-8 bottom-[-24px] w-0.5 bg-gray-100 dark:bg-gray-800" />}
