...
export default function EmployeeTicketHistorySidebar({
  ...
  return (
    ...
    >
        ...
        <>
          <div className="grid grid-cols-4 gap-2 mb-4">
            <div className="p-3 rounded-xl border border-[var(--border)] text-center">
              <p className="text-lg font-bold text-[var(--text)] leading-none">{stats.total}</p>
              <p className="text-[10px] text-[var(--text-muted)] mt-1">Tickets</p>
            </div>
            <div className="p-3 rounded-xl border border-[var(--border)] text-center">
              <Stethoscope size={14} className="mx-auto mb-1 text-red-400" />
              <p className="text-lg font-bold text-[var(--text)] leading-none">{stats.urgences}</p>
              <p className="text-[10px] text-[var(--text-muted)] mt-1">Urgences</p>
            </div>
            <div className="p-3 rounded-xl border border-[var(--border)] text-center">
              <Briefcase size={14} className="mx-auto mb-1 text-amber-500" />
              <p className="text-lg font-bold text-[var(--text)] leading-none">{stats.missions}</p>
              <p className="text-[10px] text-[var(--text-muted)] mt-1">Missions</p>
            </div>
            <div className="p-3 rounded-xl border border-[var(--border)] text-center">
              <XCircle size={14} className="mx-auto mb-1 text-[var(--text-muted)]" />
              <p className="text-lg font-bold text-[var(--text)] leading-none">{stats.rejected}</p>
              <p className="text-[10px] text-[var(--text-muted)] mt-1">Refusés</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 mb-6">
            <div className={`p-3.5 rounded-xl border flex items-center gap-2.5 ${stats.onTimeRate == null ? 'border-[var(--border)]' : stats.onTimeRate >= 80 ? 'border-emerald-100 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-800' : 'border-amber-100 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-800'}`}>
              {stats.onTimeRate == null ? <TrendingUp size={16} className="text-[var(--text-muted)]" /> : stats.onTimeRate >= 80 ? <CheckCircle2 size={16} className="text-emerald-500" /> : <AlertTriangle size={16} className="text-amber-500" />}
              <div>
                <p className="text-xs font-bold text-[var(--text)]">{stats.onTimeRate != null ? `${stats.onTimeRate}%` : '—'}</p>
                <p className="text-[10px] text-[var(--text-muted)]">Retours à l&apos;heure</p>
              </div>
            </div>
            <div className="p-3.5 rounded-xl border border-[var(--border)] flex items-center gap-2.5">
              <Timer size={16} className="text-[var(--text-muted)]" />
              <div>
                <p className="text-xs font-bold text-[var(--text)]">{stats.avgMinutesOut != null ? `${Math.round(stats.avgMinutesOut / 60 * 10) / 10}h` : '—'}</p>
                <p className="text-[10px] text-[var(--text-muted)]">Durée moyenne dehors</p>
              </div>
            </div>
          </div>
        </>
      ...
      <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Historique ({sorted.length})</p>
      <div className="space-y-2">
        {sorted.length === 0 ? (
          <p className="text-sm text-[var(--text-muted)] text-center py-8">Aucun ticket enregistré pour cet employé.</p>
        ) : sorted.map(t => {
          ...
          return (
            <div key={t.id} className="p-3.5 rounded-xl border border-[var(--border)]">
              <div className="flex items-start justify-between gap-2 mb-1.5">
                <div className="flex items-center gap-2 min-w-0">
                  <Icon size={14} className="text-[var(--text-muted)] shrink-0" />
                  <p className="text-sm font-semibold text-[var(--text)] truncate">{TYPE_LABEL[t.type] ?? t.type}</p>
                </div>
              ...
              <p className="text-xs text-[var(--text-muted)] mb-1">{fmtDateTime(t.departureTime)} → {t.actualReturnTime ? fmtDateTime(t.actualReturnTime) : `prévu ${new Date(t.expectedReturnTime).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}`}</p>
              <p className="text-xs text-[var(--text-muted)] line-clamp-2">{t.reason}</p>
              {late && (
                <p className="text-[11px] text-amber-500 mt-1 flex items-center gap-1"><AlertTriangle size={11} /> Retour en retard</p>
              )}
