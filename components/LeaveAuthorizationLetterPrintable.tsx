...
export interface LeaveLetterData {
  ...
  // ✅ Congé ANNUAL_ANTICIPATED uniquement — purement informatif, aucun
...
export default function LeaveAuthorizationLetterPrintable({ id, data }: { id: string; data: LeaveLetterData }) {
  ...
  return (
    ...
    >
      ...
      {/* ✅ Congé anticipé — purement informatif, pas de motif ni de report
          (aucune indemnité n'est payée à la date de ce congé précis, le
          solde reste simplement disponible dans le cycle en cours). */}
      {data.availableBalanceAfter != null && (
        <p style={{ textAlign: 'justify', margin: '0 0 12px' }}>
          Il vous reste, après ce congé, <strong>{data.availableBalanceAfter} jours</strong> de congé disponibles
          dans votre solde actuel, à prendre à une date ultérieure.
        </p>
      )}
