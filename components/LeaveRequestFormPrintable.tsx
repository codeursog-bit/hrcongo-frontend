  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        ...
        <div style={{ fontSize: 11, fontWeight: 700 }}>Réf. N° : {data.reference || '.....................'}</div>
      </div>

      <div style={{ border: '2px solid #1f2937', padding: '10px 16px', marginBottom: 22 }}>
        <h1 style={{ fontSize: 17, fontWeight: 800, letterSpacing: 1, margin: 0, textAlign: 'center' }}>DEMANDE DE CONGÉ</h1>
      </div>
      ...
      <div style={{ fontSize: 13, lineHeight: 2.1 }}>
        ...
        <div style={{ marginTop: 8 }}><strong>Type de congé :</strong> {TYPE_LABEL[data.type] ?? data.type}</div>

        {data.reason && <div style={{ marginTop: 6 }}><strong>Motif :</strong> {data.reason}</div>}
      ...
      <div style={{ marginTop: 26, fontSize: 13 }}>
        ...
        {data.status === 'REJECTED' && data.rejectionReason && (
          <div style={{ marginTop: 8, fontSize: 12, fontStyle: 'italic' }}>Motif du refus : {data.rejectionReason}</div>
        )}
      </div>
      ...
      <div style={{ marginTop: 34, border: '1.5px solid #1f2937', display: 'flex' }}>
        {[
          ...
          <div key={col.label} style={{ flex: 1, padding: '14px 10px 18px', borderLeft: i === 0 ? 'none' : '1.5px solid #1f2937', minHeight: 130, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', textAlign: 'center' }}>
