...
// ✅ WHITE-LABEL : accepte des props optionnelles brandName / brandLogo / brandColor
// → Si fournis (mode PME cabinet), affiche le logo/nom du cabinet à la place de Konza
// → Si absents (mode Konza normal), comportement identique à avant
...
interface SidebarProps {
  isOpen: boolean;
  onClose: () => void;
  // ✅ Props white-label (optionnelles — uniquement fournies depuis le layout PME)
  brandName?:  string | null;   // Nom du cabinet, ex: "Cabinet Nguesso & Associés"
  brandLogo?:  string | null;   // URL du logo cabinet (stocké dans cabinet.logo)
  brandColor?: string | null;   // Couleur primaire du cabinet (ex: "#06b6d4")
  // Base path pour construire les liens (défaut: '' = /dashboard, /employes…)
  // En mode PME : '/pme/[companyId]' → liens deviennent /pme/[id]/employes…
  basePath?: string;
}
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Mode white-label : au moins un des props brand est fourni
  ...
  // En mode white-label, les chemins sont basePath + item.path
  // Ex: basePath='/pme/abc123', item.path='/dashboard' → '/pme/abc123/dashboard'
  const buildPath = (itemPath: string) => {
    if (!basePath) return itemPath;
    // Retire le slash initial du itemPath pour éviter le double slash
  ...
  return (
    <>
      ...
      <aside
        ...
        `}
      >
        {/* Glow line top — couleur du cabinet en mode white-label */}
        <div
          className="absolute top-0 left-0 right-0 h-px opacity-0 dark:opacity-60"
          style={{
        ...
        <div className="px-6 pt-5 pb-2">
          {isWhiteLabel ? (
            /* ── Mode WHITE-LABEL : logo/nom du cabinet ── */
            <div className="flex flex-col items-start">
              <div className="flex items-center gap-3">
                {brandLogo ? (
                  /* Logo image du cabinet */
                  <img
                    src={brandLogo}
                    alt={brandName ?? 'Cabinet'}
                    style={{ height: 36, width: 'auto', maxWidth: 150, objectFit: 'contain' }}
                  />
                ) : (
                  /* Fallback : icône colorée avec les initiales */
                  <div
                  ...
                  >
                    ...
                    >
                      {(brandName ?? 'C').slice(0, 2).toUpperCase()}
                    </span>
                  </div>
                )}

                {/* Nom du cabinet à côté si pas de logo image */}
                {!brandLogo && brandName && (
                  <p
                    className="font-black text-sm leading-tight"
                    style={{ color: accentColor, maxWidth: 160 }}
                  >
                    {brandName}
                  </p>
                )}
            ...
            <Link href="/dashboard" className="inline-flex flex-col items-start group">
              ...
              <div className="flex items-center gap-1.5 mt-1.5 pl-1">
                ...
                <span className="text-[10px] text-slate-500 dark:text-cyan-400 font-bold tracking-widest uppercase">
                  Online
                </span>
              </div>
            </Link>
            ...
            <div className="relative shrink-0">
              {user ? (
                <img
                  src={user.avatarUrl}
                  alt={user.name}
                  className="w-10 h-10 rounded-xl object-cover ring-2 ring-white dark:ring-white/10 shadow-sm"
                />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              {user ? (
                <>
                  <h3 className="text-sm font-bold text-slate-800 dark:text-white truncate">{user.name}</h3>
                  <span
                    className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-white dark:bg-white/10 border border-slate-100 dark:border-white/5 shadow-sm"
                    style={{ color: isWhiteLabel ? accentColor : undefined }}
                    // En mode Konza on garde la classe originale
                    {...(!isWhiteLabel && { className: 'inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-white dark:bg-white/10 text-sky-600 dark:text-cyan-300 border border-slate-100 dark:border-white/5 shadow-sm' })}
                  >
                    {roleLabels[user.role] || user.role}
                  </span>
                </>
            ...
            .map(item => {
              ...
              return (
                <Link
                  ...
                  `}
                  style={active ? {
                    // Mode white-label : couleur du cabinet ; mode Konza : sky/cyan
                    color:      isWhiteLabel ? accentColor : undefined,
                    background: isWhiteLabel ? `${accentColor}12` : undefined,
                    borderColor:isWhiteLabel ? `${accentColor}40` : undefined,
                  } : {}}
                  {...(active && !isWhiteLabel && {
                    className: `relative w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-300 group overflow-hidden
                      text-sky-700 dark:text-white bg-sky-50 dark:bg-white/5 border border-sky-100 dark:border-cyan-500/30 shadow-sm dark:shadow-none`,
                  })}
                >
                  {/* Barre active gauche */}
                  {active && (
                    <div
                      ...
                      style={{
                        background:  isWhiteLabel ? accentColor : '#0ea5e9',
                  ...
                  <item.icon
                    ...
                    `}
                    style={active ? { color: isWhiteLabel ? accentColor : undefined } : {}}
                  />
                  <span className="relative z-10">{item.label}</span>
                </Link>
              );
        ...
        <div className="p-4 border-t border-slate-100 dark:border-white/5">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all border border-transparent hover:border-red-100 dark:hover:border-red-500/20 group"
          >
            <LogOut size={18} className="group-hover:-translate-x-1 transition-transform" />
            <span>Déconnexion</span>
          </button>
