                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <button onClick={() => onSelectEmployee(r.employee)} className="font-semibold text-gray-800 dark:text-gray-100 hover:text-sky-600 hover:underline truncate">
                          {r.employee?.firstName} {r.employee?.lastName}
                        </button>
                        {r.employee?.department?.name && <p className="text-xs text-gray-400 truncate">{r.employee.department.name}</p>}
                      </div>
                    </div>
                  </td>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <div className="divide-y divide-gray-100 dark:divide-gray-700">
          {recentlyValidatedLoans.length === 0 ? (
            ...
            <button key={l.id} onClick={() => onSelectEmployee(l.employee)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/40 text-left">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate">{l.employee?.firstName} {l.employee?.lastName}</p>
...
function KpiCard({ icon: Icon, label, value, color }: { icon: any; label: string; value: string; color: 'sky' | 'emerald' | 'violet' | 'amber' }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4 hover:shadow-md transition-shadow">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 bg-gradient-to-br text-white shadow-lg ${cls[color]}`}><Icon size={18} /></div>
...
function StatusPill({ status }: { status: string }) {
  const cfg: Record<string, { label: string; cls: string; dot: string }> = {
    ...
    PAID: { label: 'Remboursé(e)', cls: 'bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-300', dot: 'bg-sky-500' },
    DEDUCTED: { label: 'Déduite', cls: 'bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-300', dot: 'bg-sky-500' },
    REJECTED: { label: 'Refusé(e)', cls: 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-300', dot: 'bg-red-500' },
    CANCELLED: { label: 'Annulé(e)', cls: 'bg-gray-100 text-gray-500 dark:bg-gray-700/50 dark:text-gray-400', dot: 'bg-gray-400' },
  };
  const c = cfg[status] ?? { label: status, cls: 'bg-gray-100 text-gray-500', dot: 'bg-gray-400' };
