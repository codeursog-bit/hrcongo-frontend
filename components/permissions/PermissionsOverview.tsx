...
const TYPE_COLOR: Record<string, string> = { URGENCE: '#ef4444', MISSION: '#10b981', AUTRE: '#94a3b8' };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={Clock} label="Tickets ce mois-ci" value={String(kpis.countThisMonth)} tone="emerald" />
        <KpiCard icon={CheckCircle2} label="Autorisés ce mois-ci" value={String(kpis.approvedThisMonth)} tone="emerald" />
        <KpiCard icon={TrendingUp} label="Autorisés (toutes périodes)" value={String(kpis.totalApproved)} tone="amber" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-[var(--surface-2)]">
              <tr>{['Employé', 'Type', 'Sortie', 'Statut', ''].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-bold text-[var(--text-muted)] uppercase whitespace-nowrap">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-[var(--border)]">
              {displayed.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-10 text-[var(--text-muted)]">Aucune demande pour ce filtre.</td></tr>
              ) : displayed.map(t => (
                <tr key={t.id} className="hover:bg-[var(--surface-2)]">
                  <td className="px-4 py-3">
                    <button onClick={() => onSelectEmployee(t.employee)} className="font-semibold text-[var(--text)] hover:text-emerald-600 hover:underline">
                      {t.employee?.firstName} {t.employee?.lastName}
                    </button>
                    {t.employee?.department?.name && <p className="text-xs text-[var(--text-muted)]">{t.employee.department.name}</p>}
                  </td>
                  <td className="px-4 py-3 text-[var(--text-muted)]">{TYPE_LABEL[t.type] ?? t.type}</td>
                  <td className="px-4 py-3 text-[var(--text-muted)] whitespace-nowrap">{new Date(t.departureTime).toLocaleDateString('fr-FR')}</td>
                  <td className="px-4 py-3"><StatusPill status={t.status} /></td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => onGoToTicket(t.id)} className="text-emerald-600 hover:underline text-xs font-semibold flex items-center gap-0.5 ml-auto">Détail <ChevronRight size={12} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] overflow-hidden">
        <div className="p-4 border-b border-[var(--border)]">
          <p className="text-sm font-bold text-[var(--text-muted)]">Derniers tickets autorisés</p>
        </div>
        <div className="divide-y divide-[var(--border)]">
          {recentlyApproved.length === 0 ? (
            <p className="text-center py-10 text-[var(--text-muted)] text-sm">Aucun ticket autorisé pour le moment.</p>
          ) : recentlyApproved.map(t => (
            <button key={t.id} onClick={() => onSelectEmployee(t.employee)} className="w-full flex items-center justify-between gap-3 px-4 py-3 hover:bg-[var(--surface-2)] text-left">
              <div>
                <p className="text-sm font-semibold text-[var(--text)]">{t.employee?.firstName} {t.employee?.lastName}</p>
                <p className="text-xs text-[var(--text-muted)]">{TYPE_LABEL[t.type] ?? t.type} · autorisé le {new Date(t.reviewedAt ?? t.createdAt).toLocaleDateString('fr-FR')}</p>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link href={bp('/presences/permissions/suivi')} className="group relative overflow-hidden rounded-2xl p-5 bg-emerald-500 text-white hover:bg-emerald-600 transition-colors">
        ...
        <Link href={bp('/presences/permissions/validations')} className="group relative overflow-hidden rounded-2xl p-5 bg-amber-500 text-white hover:bg-amber-600 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-bold text-[var(--text-muted)]">Tickets demandés vs autorisés par mois</p>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={monthlySeries}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="Demandés" stroke="#F59E0B" strokeWidth={2.5} dot={false} />
        ...
        <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-5">
          <p className="text-sm font-bold text-[var(--text-muted)] mb-4">Répartition par type de ticket</p>
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={byType} dataKey="value" nameKey="name" innerRadius={55} outerRadius={90} paddingAngle={2}>
                {byType.map((t, i) => <Cell key={i} fill={TYPE_COLOR[t.type] ?? '#10B981'} />)}
              </Pie>
              <Tooltip />
              <Legend />
            </PieChart>
        ...
        <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-5">
          <p className="text-sm font-bold text-[var(--text-muted)] mb-4">Top 5 départements par nombre de tickets</p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={byDept} layout="vertical" margin={{ left: 24 }}>
              ...
              <Tooltip />
              <Bar dataKey="nombre" fill="#10B981" radius={[0, 6, 6, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
...
function KpiCard({ icon: Icon, label, value, tone }: { icon: any; label: string; value: string; tone: 'emerald' | 'amber' }) {
...
function StatusPill({ status }: { status: string }) {
  const cfg: Record<string, { label: string; cls: string }> = {
    ...
    CANCELLED: { label: 'Annulé', cls: 'bg-[var(--surface-2)] text-[var(--text-muted)]' },
  };
  const c = cfg[status] ?? { label: status, cls: 'bg-[var(--surface-2)] text-[var(--text-muted)]' };
