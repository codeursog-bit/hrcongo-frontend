...
const CompanyReminderContext = createContext<CompanyReminderContextType>({
  hasCompany: true, // Forcé à true pour désactiver la fonctionnalité
  setHasCompany: () => {},
  recheckCompany: async () => {},
});
...
export const CompanyReminderProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  // Fonctionnalité désactivée temporairement : on retourne simplement les children
  // sans timer, sans appel API et sans modal.
  return (
    <CompanyReminderContext.Provider value={{ hasCompany: true, setHasCompany: () => {}, recheckCompany: async () => {} }}>
      {children}
    </CompanyReminderContext.Provider>
  );
};